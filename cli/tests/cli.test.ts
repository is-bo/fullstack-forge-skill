...
test("compiled CLI exposes version, list, and blocked command execution", async () => {
  ...
  const parsed = JSON.parse(list.stdout) as {
    modules: string[];
    tools: string[];
    tool_availability: { installed_runtime: string[]; source_checkout_only: string[] };
  };
...
test("composition loads only direct dependencies reachable through observed applicability", async () => {
  await withTemporaryProject("cli-compose-applicable-dependencies", async (root) => {
    ...
    const result = await runFile(
      process.execPath,
      [cli, "frontend", "compose", "--root", root, "--dry-run", "--json"],
      root
    );
    assert.equal(result.exitCode, 0, result.stderr);
    assert.ok(
      result.stdout.length < 32_000,
      `bounded composition output unexpectedly grew to ${result.stdout.length} bytes`
    );
    const parsed = JSON.parse(result.stdout) as {
      compositions: Array<{ module: string }>;
      ...
      dependency_edges: Array<{ parent: string; dependency: string }>;
      composition_artifact?: string;
    };
    ...
    assert.equal(
      parsed.module_decisions.some((decision) => decision.module === "database"),
      false,
      "compact dependency provenance must not emit unrelated transitive decisions"
    );
...
test("explicit greenfield frontend composition includes the complete experience team", async () => {
  await withTemporaryProject("cli-compose-greenfield-frontend", async (root) => {
    ...
    const parsed = JSON.parse(result.stdout) as {
      compositions: Array<{
        module: string;
        selected: Array<{ provider: string; skill: string }>;
      }>;
    };
    const selected = new Map(
      parsed.compositions.map((composition) => [composition.module, composition])
    );
    ...
    assert.ok(
      selected
        .get("ui")
        ?.selected.some(
          (source) => source.provider === "impeccable" && source.skill === "impeccable"
        ),
      "greenfield UI composition must include Impeccable"
    );
    for (const excluded of ["database", "auth", "payments", "ship"])
      assert.equal(selected.has(excluded), false, `did not expect unrelated ${excluded} context`);
    ...
    assert.equal(archiveEquivalent.exitCode, 0, archiveEquivalent.stderr);
    assert.deepEqual(
      (JSON.parse(archiveEquivalent.stdout) as typeof parsed).compositions,
      parsed.compositions
    );
  });
});
...
test("high-risk all audit and verify route through applicable focused modules", async () => {
  await withTemporaryProject("cli-verify-all", async (root) => {
    ...
    assert.equal(
      audit.exitCode,
      2,
      "selected high-risk modules with remaining NOT_VERIFIED evidence must keep Audit incomplete"
    );
...
test("section Verify exit status ignores unrelated carried findings while all remains global", async () => {
  await withTemporaryProject("cli-verify-section-exit", async (root) => {
    ...
    const revision = await workingTreeRevision(root);
    const finding = (section: string, status: Finding["status"]): Finding => ({
    ...
    await writeReport(
      createReport(
        root,
        profile,
        [finding("security", "PASS"), finding("payments", "FAIL")],
        "mixed fixture",
        [],
        [],
        [],
    ...
    const scoped = await runFile(
      process.execPath,
      [cli, "security", "verify", "--root", root, "--json"],
      root
    );
    assert.equal(scoped.exitCode, 0, scoped.stderr);
    const scopedFindings = (
      JSON.parse(scoped.stdout) as {
        report: { findings: Array<{ section: string; status: string }> };
      }
    ).report.findings;
    assert.ok(
      scopedFindings.some(
        (candidate) => candidate.section === "payments" && candidate.status === "FAIL"
      ),
      "section Verify must preserve unrelated history without letting it determine the exit code"
    );

    const global = await runFile(
      process.execPath,
      [cli, "all", "verify", "--root", root, "--json"],
      root
    );
