 ...
 */
async function securityIds(name: string, file: string, source: string): Promise<Set<string>> {
...
`
  ...
  assert.ok(
    ids.has("FF-SEC-SQL-001"),
    "a request value aliased through a local const and a template literal must still reach the SQL sink"
  );
});
...
test("same-file function-parameter propagation reaches a SQL sink", async () => {
  const ids = await securityIds(
    "flow-parameter",
    "repo.ts",
    `export function loadUser(id, db) {
...
test("unrelated validation does not suppress a proven injection finding", async () => {
  const ids = await securityIds(
    "flow-unrelated-validation",
    "handler.ts",
    `function validateUnrelatedConfiguration() { return true; }
...
`
  ...
  assert.ok(
    ids.has("FF-SEC-VALIDATION-001"),
    "an unrelated validate* call must not count as validating this value"
  );
});
...
test("taint evidence records the propagation path from source to sink", () => {
  ...
  assert.ok(
    origin.steps.some((step) => step.includes("template literal")),
    `expected a template-literal propagation step, got ${JSON.stringify(origin.steps)}`
  );
});
