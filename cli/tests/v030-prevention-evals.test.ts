...
type Check = {
  id: string;
  classification:
    "deterministic" | "nondeterministic" | "human-required" | "unsupported-external-tool";
  status: "PASS" | "NOT_VERIFIED" | "BLOCKED";
};
type PreventionCase = {
  id: (typeof EXPECTED_IDS)[number];
  title: string;
  agent_task: string;
  starting_repository: { file_map: Array<{ path: string; content: string }> };
  expected_applicability: string[];
  machine_required: string[];
  expected_gates: BuildGateId[];
  forbidden_defects: string[];
  required_validation_artifacts: string[];
  checks: Check[];
};
...
test("every prevention case is safe to materialize and classifies unavailable evidence honestly", async (t) => {
  for (const entry of await loadCases()) {
    await t.test(entry.id, async () => {
      ...
      assert.deepEqual(
        new Set(entry.checks.map((check) => check.classification)),
        CLASSIFICATIONS,
        "each case must distinguish local proof from unavailable evidence"
      );
      for (const check of entry.checks) {
        assert.ok(STATUSES.has(check.status));
        if (check.classification === "deterministic") assert.equal(check.status, "PASS");
        else assert.notEqual(check.status, "PASS", `${check.id} cannot be represented as PASS`);
      }
  ...
  for (const entry of await loadCases()) {
    await t.test(entry.id, () => {
      ...
      for (const discipline of entry.machine_required)
        assert.ok(
          applicability.required.includes(discipline),
          `${entry.id}: ${discipline} must be required by exported applicability logic`
        );
      ...
      for (const id of entry.expected_gates)
        assert.ok(
          plan.gates.some((gate) => gate.id === id),
          `${entry.id}: missing ${id}`
        );
...
test("reject-unnecessary-redis excludes an absent cache rather than inventing a cache pass", async () => {
  const entry = (await loadCases()).find(
    (candidate) => candidate.id === "reject-unnecessary-redis"
  );
  assert.ok(entry);
  const applicability = deriveBuildApplicability({
    profile,
...
test("hostile AI input and stale evidence cannot become a PASS without the required artifacts", async () => {
  const entry = (await loadCases()).find(
    (candidate) => candidate.id === "hostile-ai-invoice-extraction"
  );
  assert.ok(entry);
  const plan = planBuildGates({
    tier: "high",
  ...
  const authorization = evaluateBuildGates(plan, []).find(
    (gate) => gate.id === "FF-BUILD-GATE-AUTHORIZATION-NEGATIVE"
  );
...
async function loadCases(): Promise<PreventionCase[]> {
  const value: unknown = JSON.parse(await readFile(CASES_PATH, "utf8"));
  assert.equal(typeof value, "object");
  assert.ok(value !== null && !Array.isArray(value));
  const corpus = value as { schema_version?: unknown; cases?: unknown };
  assert.equal(corpus.schema_version, 1);
  assert.ok(Array.isArray(corpus.cases));
  return corpus.cases.map((entry) => assertCase(entry));
}

function assertCase(value: unknown): PreventionCase {
  assert.equal(typeof value, "object");
  assert.ok(value !== null && !Array.isArray(value));
  const entry = value as PreventionCase;
...
function applicabilityFor(required: string[]): BuildApplicabilityResult {
  return { decisions: [], required, suggested: [], unresolved: [], excluded: [] };
}

function evidence(criterion: string, path: string, source: string): CriterionEvidence {
  return {
    criterion,
