...
async function authorizationFindings(
  name: string,
...
async function objectOutcomes(name: string, files: Record<string, string>): Promise<Set<string>> {
...
const route = (importLine: string, guard: string, body?: string) =>
  `${importLine}
...
test("the object-authorization decision table is total and fail-closed", () => {
  const authorities: AdministrativeAuthority[] = ["global", "tenant", "ambiguous", "none"];
  const partitions: ResourcePartition[] = ["global", "partitioned"];
  const observed: string[] = [];
  for (const authority of authorities)
    for (const partition of partitions) {
      observed.push(
        `${authority}/${partition}=${decideObjectAuthorization({ boundPredicate: false, authority, partition }).outcome}`
      );
      assert.equal(
        decideObjectAuthorization({ boundPredicate: true, authority, partition }).outcome,
        "authorized",
        "a predicate bound to the object always wins"
      );
    }
...
test("an unread guard body never yields platform authority", () => {
  ...
  assert.equal(
    classifyAdministrativeAuthority({ resolved: false, text: "requireSuperAdmin" }, roles),
    "ambiguous"
  );
  assert.equal(
    classifyAdministrativeAuthority({ resolved: true, text: 'role !== "superadmin"' }, roles),
    "global"
  );
  assert.equal(
    classifyAdministrativeAuthority({ resolved: true, text: 'role !== "orgAdmin"' }, roles),
    "tenant"
  );
  assert.equal(
    classifyAdministrativeAuthority({ resolved: true, text: 'role !== "admin"' }, roles),
    "ambiguous"
  );
  assert.equal(
    classifyAdministrativeAuthority({ resolved: true, text: 'role !== "editor"' }, roles),
    "none"
  );
});
...
test("a narrower partition administrator is never widened by a nearby global role name", () => {
  const authority = classifyAdministrativeAuthority(
    { resolved: true, text: 'requireOrgAdmin superadmin role !== "orgAdmin"' },
    new Set()
  );
...
test("an explicit role mapping is the only way a plain admin role becomes global", () => {
  ...
  assert.equal(
    classifyAdministrativeAuthority({ resolved: true, text: 'role !== "admin"' }, new Set()),
    "ambiguous"
  );
});
...
test("a proven platform administrator on unpartitioned state is a policy note, not a defect", async () => {
  ...
  assert.ok(
    outcomes.has(`${ADMINISTRATIVE}=WARNING`),
    "the reliance on a role grant is still published"
  );
});
...
test("a tenant administrator deleting by global identifier is still a defect", async () => {
  ...
  assert.ok(
    outcomes.has(`${OBJECT}=FAIL`),
    "an organisation administrator proves nothing about another organisation's object"
  );
...
test("a role guard that is not administrative is never suppressed", async () => {
  ...
  assert.ok(
    outcomes.has(`${OBJECT}=FAIL`),
    "a role gate without proof of global object scope must not clear the object rule"
  );
});
...
test("an unresolvable external administrator guard is never proven global", async () => {
  ...
  assert.ok(
    outcomes.has(`${OPEN}=NOT_VERIFIED`),
    "a package name is not proof of platform-wide reach"
  );
  assert.ok(!outcomes.has(`${ADMINISTRATIVE}=WARNING`));
});
...
test("a platform administrator over partitioned data stays NOT_VERIFIED", async () => {
  const outcomes = await objectOutcomes("object-authz-global-admin-partitioned", {
    ...
    "routes.ts": route(
      ...
      "requireSuperAdmin, ",
      "  audit(req.query.tenantId);\n"
    )
  });
  assert.ok(
    outcomes.has(`${OPEN}=NOT_VERIFIED`),
    "nothing proves a platform role spans every partition of a partitioned resource"
  );
  assert.ok(!outcomes.has(`${ADMINISTRATIVE}=WARNING`));
});
...
test("object-authorization findings are identical across repeated runs", async () => {
  ...
  assert.deepEqual(
    first.map((finding) => [finding.id, finding.instance_id, finding.status, finding.evidence]),
    second.map((finding) => [finding.id, finding.instance_id, finding.status, finding.evidence])
  );
});
