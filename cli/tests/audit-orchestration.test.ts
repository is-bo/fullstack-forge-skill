...
function command(name: string, definition: string): CommandDefinition {
...
const succeedingRunner: CommandRunner = () =>
  Promise.resolve({ exitCode: 0, stdout: "ok", stderr: "" });

function baseInput(overrides: Partial<AuditOrchestrationInput> = {}): AuditOrchestrationInput {
  return {
    root: "/repo",
    modules: ["security"] as ModuleSlug[],
    commands: COMMANDS,
    allowRun: false,
    offline: false,
    dryRun: false,
...
function ids(checks: PlannedAuditCheck[]): string[] {
...
test("a static-only security audit plans modules and never executes a command", async () => {
  ...
  assert.deepEqual(
    ids(result.planned).filter((id) => id.startsWith("module:")),
    ["module:security"]
  );
...
test("the planned-check list never includes an unknown project server script", () => {
  const planned = buildAuditPlan({ modules: ["ui"] as ModuleSlug[], commands: COMMANDS });
  assert.equal(
    planned.some((check) => check.name === "start" || check.name === "dev"),
    false
  );
  for (const check of planned) {
    if (check.kind !== "project-command") continue;
    assert.ok((CANDIDATE_PROJECT_CHECKS as readonly string[]).includes(check.name));
  }
});

test("planned-check order is deterministic regardless of input ordering", () => {
  const first = buildAuditPlan({
    modules: ["ui", "api", "security"] as ModuleSlug[],
    commands: COMMANDS,
    url: "http://127.0.0.1:3000/"
  });
  const second = buildAuditPlan({
    modules: ["security", "ui", "api"] as ModuleSlug[],
...
test("no arbitrary project command can reach OFFLINE_SAFE, and a blocked check never passes", async () => {
  for (const definition of ["eslint .", "tsc --noEmit", "node build/index.js", "echo ok"]) {
    assert.notEqual(
      commandNetworkPolicy(command("check", definition), { offline: true, forgeOwned: false }),
      "OFFLINE_SAFE",
      `'${definition}' must never be classified offline-safe`
    );
  }
  const ledger = new ReportAuditLedger();
  await orchestrateAudit(
    baseInput({ allowRun: true, offline: true, ledger, commands: [command("lint", "eslint .")] })
  );
...
test("an unknown check name is rejected instead of silently ignored", async () => {
  await assert.rejects(
    () => orchestrateAudit(baseInput({ select: ["lnit"] })),
    /Unknown --check value 'lnit'/u
  );
  await assert.rejects(
    () => orchestrateAudit(baseInput({ skip: ["module:nonexistent"] })),
    /Unknown --skip-check value/u
  );
});
...
test("a supplied URL collects rendered evidence and records it", async () => {
  const ledger = new ReportAuditLedger();
  const collector: RuntimeEvidenceCollector = (input) =>
    Promise.resolve({
      kind: "rendered-ui",
      status: "COMPLETE",
      url: input.url,
...
test("a UI audit and an all-module audit plan the modules they were given", async () => {
  const ui = await orchestrateAudit(baseInput({ modules: ["ui"] as ModuleSlug[] }));
  assert.deepEqual(
    ids(ui.planned).filter((id) => id.startsWith("module:")),
    ["module:ui"]
  );
  const all = await orchestrateAudit(
    baseInput({ modules: ["api", "security", "ui"] as ModuleSlug[] })
  );
  assert.deepEqual(
    ids(all.planned).filter((id) => id.startsWith("module:")),
    ["module:api", "module:security", "module:ui"]
  );
});

test("a changed-scope audit plans only the modules the scope selected", async () => {
  const result = await orchestrateAudit(baseInput({ modules: ["queries"] as ModuleSlug[] }));
  assert.deepEqual(
    ids(result.planned).filter((id) => id.startsWith("module:")),
    ["module:queries"]
  );
