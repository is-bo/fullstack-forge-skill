...
test("secret scanner ignores low-confidence assignments in vendored guidance but keeps high-signal detection", async () => {
  await withTemporaryProject("secret-vendored-guidance", async (root) => {
    ...
    await writeFile(
      ...
      'client_secret = "documented-test-runtime-variable-value"\n',
      "utf8"
    );
    ...
    assert.ok(
      result.findings.every((finding) =>
        finding.location.every((location) => !location.path.endsWith("configuration.md"))
      )
    );
    assert.ok(
      result.observations.every((observation) => !observation.path.endsWith("configuration.md"))
    );
  });
});
