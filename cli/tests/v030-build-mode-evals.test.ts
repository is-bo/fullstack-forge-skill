...
import { evaluateBuildGates, planBuildGates, type BuildGateId } from "../src/build-gates.js";
...
type ExpectedStatus = "PASS" | "FAIL" | "BLOCKED" | "NOT_VERIFIED";
type Fixture = {
  schema_version: 1;
...
const emptyApplicability: BuildApplicabilityResult = {
  decisions: [],
  required: [],
  suggested: [],
  unresolved: [],
  ...
  for (const fixture of await loadFixtures()) {
    await t.test(fixture.id, async () => {
      ...
      assert.equal(
        ...
        `${fixture.id} must preserve its declared status`
      );
    });
  }
});
...
async function loadFixtures(): Promise<Fixture[]> {
  ...
  assert.equal(
    entries.length,
    files.length,
    "v0.3 corpus fixtures must be regular JSON files only"
  );
...
function assertFixture(value: unknown, file: string): asserts value is Fixture {
  ...
  assert.ok(typeof fixture.id === "string" && EXPECTED_IDS.includes(fixture.id as Fixture["id"]));
  ...
  assert.ok(
    typeof fixture.expected_status === "string" && EXPECTED_STATUSES.has(fixture.expected_status),
    `${file} must declare an asserted outcome`
  );
  assert.ok(typeof fixture.task === "string" && fixture.task.length > 30);
  if (fixture.disciplines !== undefined) {
    assert.ok(
      Array.isArray(fixture.disciplines) &&
        fixture.disciplines.every((item) => typeof item === "string")
    );
  }
  if (fixture.gate_id !== undefined) {
    const gateId = fixture.gate_id;
    if (typeof gateId !== "string") assert.fail(`${file} gate_id must be a string`);
    assert.match(gateId, /^FF-BUILD-GATE-/u);
  }
}

async function runFixture(fixture: Fixture): Promise<ExpectedStatus> {
  switch (fixture.kind) {
    case "producer-pass":
...
async function producerStatus(
  ...
  if (!allowRun) assert.equal(observation.command.exit_code, undefined);
  return observation.status as ExpectedStatus;
}
...
function uiRuntimeMatrixStatus(): ExpectedStatus {
  ...
  for (const state of BUILD_RUNTIME_STATES)
    assert.equal(
      plan.cases.filter((entry) => entry.state === state).length,
      BUILD_RUNTIME_VIEWPORTS.length
    );
  const rendered: RenderedRuntimeCapture = {
...
function requiredGateStatus(fixture: Fixture): ExpectedStatus {
  ...
  return gate.status as ExpectedStatus;
}

function migrationGatesStatus(): ExpectedStatus {
  const plan = planBuildGates({
    tier: "high",
    commands: [],
  ...
  const gates = evaluateBuildGates(plan, evidence).filter(
    (entry) =>
      entry.id === "FF-BUILD-GATE-MIGRATION" || entry.id === "FF-BUILD-GATE-MIGRATION-RECOVERY"
  );
...
function criterion(name: string): CriterionEvidence {
  return {
    criterion: name,
...
async function staleAndForgedEvidenceStatus(): Promise<ExpectedStatus> {
  return withTemporaryProject<ExpectedStatus>("v030-stale-forged", async (root) => {
...
async function migrationRollbackStatus(): Promise<ExpectedStatus> {
  return withTemporaryProject<ExpectedStatus>("v030-migration", async (root) => {
...
function legacyProject(): Record<string, unknown> {
  const project = { ...newProject("synthetic legacy project", "standard") } as Record<
    string,
    unknown
  >;
...
function legacyFeature(): Record<string, unknown> {
