...
import type { PlannedCheck, RuntimeEvidence, ToolRecord } from "../src/types.js";

const HASH_A = "a".repeat(64);
const HASH_B = "b".repeat(64);

function plan(id: string, overrides: Partial<PlannedCheck> = {}): PlannedCheck {
...
function evidence(id: string, overrides: Partial<RuntimeEvidence> = {}): RuntimeEvidence {
  return {
    evidence_id: id,
...
test("planned checks deduplicate by check_id and preserve append order", () => {
  ...
  assert.deepEqual(
    ledger.map((check) => check.check_id),
    ["FF-CHK-001", "FF-CHK-002"]
  );
...
test("a blocked check can never be re-recorded as run", () => {
  ...
  assert.throws(
    () => recordExecutedCheck(ledger, "FF-CHK-001"),
    /cannot be re-recorded as RUN/u,
    "a block that later reports a result would erase the block"
  );
  assert.throws(
    () => appendPlannedCheck(ledger, plan("FF-CHK-001", { status: "RUN" })),
    /cannot be re-recorded as RUN/u
  );
...
test("planned checks record authorization and network policy", () => {
  ...
  assert.throws(
    () => plan("FF-CHK-BAD", { network_policy: "SOMETIMES" as never }),
    /invalid network_policy/u
  );
});

test("runtime evidence merges artifacts and hashes without upgrading a blocked result", () => {
  let ledger = appendRuntimeEvidence(
    [],
    evidence("EV-1", { status: "BLOCKED", limitations: ["Browser launch was refused."] })
  );
  ledger = appendRuntimeEvidence(
    ledger,
    evidence("EV-1", {
      status: "BLOCKED",
...
test("partial rendered evidence is representable but must declare its limitations", () => {
  ...
  assert.throws(
    () => appendRuntimeEvidence([], evidence("EV-SILENT", { status: "NOT_VERIFIED" })),
    /requires at least one limitation/u,
    "a partial capture with no stated limitation reads as a clean result"
  );
});

test("runtime evidence rejects unsafe artifact paths and malformed hashes", () => {
  assert.throws(
    () => appendRuntimeEvidence([], evidence("EV-ESCAPE", { artifact_paths: ["../secrets.env"] })),
    /safe repository-relative paths/u
  );
  assert.throws(
    () => appendRuntimeEvidence([], evidence("EV-ABS", { artifact_paths: ["/etc/passwd"] })),
    /safe repository-relative paths/u
  );
  assert.throws(
    () => appendRuntimeEvidence([], evidence("EV-HASH", { hashes: ["not-a-hash"] })),
    /lowercase sha256 digests/u
  );
  assert.throws(
    () => appendRuntimeEvidence([], evidence("EV-HASH2", { hashes: [HASH_A.toUpperCase()] })),
    /lowercase sha256 digests/u
  );
  assert.doesNotThrow(() =>
    appendRuntimeEvidence([], evidence("EV-PREFIX", { hashes: [`sha256:${HASH_A}`] }))
  );
});
...
test("module decisions deduplicate by module and merge their reasons", () => {
  ...
  assert.throws(
    () =>
      appendModuleDecision(ledger, {
        module: "api",
        capability_status: "MAYBE" as never,
...
test("tool records distinguish ownership, trust, and an unknown version", () => {
  ...
  assert.deepEqual(
    ledger.map((tool) => `${tool.ownership}/${tool.trust}`),
    ["forge-owned/trusted", "project-owned/unknown", "external/untrusted"]
  );
...
test("ledger appends are deterministic for identical inputs", () => {
  ...
  assert.deepEqual(
    build().map((check) => check.check_id),
    ["FF-C", "FF-A", "FF-B"]
  );
});
