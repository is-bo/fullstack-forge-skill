...
} from "../src/repository-inventory.js";
...
test("generated-output-heavy repositories do not spend the text budget or activate capabilities", async () => {
  await withTemporaryProject("inventory-generated", async (root) => {
    ...
    assert.equal(
      inventory.entries.find((entry) => entry.path === "src/build/domain.ts")?.status,
      "INSPECTED"
    );
  });
});
...
test("Python environments and bytecode remain outside application evidence", async () => {
  await withTemporaryProject("inventory-python", async (root) => {
    ...
    assert.ok(
      profile.inventory.default_paths_excluded + profile.inventory.generated_paths_excluded >= 3
    );
  });
});
...
test("many small binaries do not consume the relevant-text budget", async () => {
  await withTemporaryProject("inventory-binary", async (root) => {
    ...
    assert.equal(
      inventory.diagnostics.bytes_read,
      Buffer.byteLength("export const inspected = true;")
    );
...
test("runtime data is excluded visibly without loading private content", async () => {
  await withTemporaryProject("inventory-runtime", async (root) => {
    ...
    assert.ok(
      inventory.diagnostics.excluded_paths.every(
        (entry) => !JSON.stringify(entry).includes("must-not-appear")
      )
    );
    assert.equal(
      inventory.entries.some((entry) => entry.path.includes("private.sqlite")),
      false
    );
  });
});
...
test("runtime-looking source is inspected when tracked or nested", async () => {
  await withTemporaryProject("inventory-runtime-source", async (root) => {
    ...
    await writeFile(
      join(root, "src", "uploads", "untracked.ts"),
...
test("fallback discovery fails closed for ambiguous top-level runtime source", async () => {
  await withTemporaryProject("inventory-runtime-fallback", async (root) => {
    ...
    await writeFile(
      join(root, "src", "uploads", "handler.ts"),
...
test("runtime-looking source excluded by .forgeignore remains incomplete evidence", async () => {
  await withTemporaryProject("inventory-runtime-forgeignore", async (root) => {
    ...
    assert.equal(inventory.diagnostics.required_evidence_excluded, true);
    assert.equal(
      inventory.entries.some((entry) => entry.path === "src/uploads/handler.ts"),
      false
    );
  });
});
...
test(".forgeignore supports reviewed patterns and makes required exclusions partial", async () => {
  await withTemporaryProject("inventory-forgeignore", async (root) => {
    ...
    await writeFile(
      join(root, ".forgeignore"),
      "# reviewed local paths\n\nprivate data\nunicode-λ\ngenerated/*\n",
      "utf8"
    );
...
test("CLI runtime-source exclusions are visible and fail closed for Audit, Verify, and Ship", async () => {
  await withTemporaryProject("inventory-cli", async (root) => {
    ...
    const auditJson = JSON.parse(audit.stdout) as {
      report: {
        profile: { inventory: { status: string; cli_exclusions: string[] } };
        environment: { inspection_budget_bytes: number; inventory_exclusions: string[] };
    ...
    assert.ok(
      auditJson.report.findings.some(
        (finding) => finding.id === "FF-INVENTORY-001" && finding.status === "NOT_VERIFIED"
      )
    );
    assert.equal(
      auditJson.report.findings.some(
        (finding) => finding.id === "FF-INVENTORY-001" && finding.status === "PASS"
      ),
      false
    );
    ...
    const verifyJson = JSON.parse(verify.stdout) as {
      report: { findings: Array<{ id: string; status: string }> };
    };
    assert.ok(
      verifyJson.report.findings.some(
        (finding) => finding.id === "FF-INVENTORY-001" && finding.status === "NOT_VERIFIED"
      )
    );
    ...
    const shipJson = JSON.parse(ship.stdout) as {
      findings: Array<{ id: string; status: string }>;
    };
    assert.ok(
      shipJson.findings.some(
        (finding) => finding.id === "FF-INVENTORY-001" && finding.status === "NOT_VERIFIED"
      )
    );
  });
...
test("Git-aware inventory respects ignores while retaining tracked generated paths and Unicode", async () => {
  await withTemporaryProject("inventory-git", async (root) => {
    ...
    await writeFile(
      join(root, "dist", "tracked source.ts"),
    ...
    assert.equal(inventory.diagnostics.status, "COMPLETE");
    assert.equal(
      inventory.entries.some((entry) => entry.path.startsWith("ignored/")),
      false
    );
    assert.equal(
      inventory.entries.find((entry) => entry.path === "dist/tracked source.ts")?.reason,
      "tracked-generated-path-neutralized"
    );
    assert.equal(
      inventory.entries.find((entry) => entry.path === "nested/λ file.ts")?.status,
      "INSPECTED"
    );
    assert.equal(
      inventory.entries.find((entry) => entry.path === "nested/untracked.ts")?.origin,
      "untracked"
    );
