...
function profile(capabilities: string[] = []): ProjectProfile {
  const detected = Object.fromEntries(
    capabilities.map((name) => [name, { name, confidence: "HIGH" as const, evidence: ["fixture"] }])
  );
  return {
...
test("documentation, test, fixture, and generated path signals cannot activate a discipline", () => {
  ...
  for (const discipline of ["auth", "uploads", "payments"])
    assert.ok(
      !result.required.includes(discipline),
      `${discipline} must not activate from non-production paths`
    );
});
