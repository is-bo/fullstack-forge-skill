...
export async function update(req, prisma) {
  return prisma.user.update({ where: { id: req.params.id }, data: ${name}(req.body) });
    ...
    assert.ok(
      ...
      `${name} must not prove a request body is field-bounded`
    );
  }
});
...
export function store(req) {
  ...
  assert.ok(
    ids.has("FF-UPLOAD-FILENAME-001"),
    "a no-op wrapper around a client-supplied filename proves nothing"
  );
});
...
export async function agent(req, model, billing) {
  ...
  assert.ok(
    ids.has("FF-AI-IRREVERSIBLE-001"),
    `a no-op wrapper must not clear an irreversible AI action; got ${[...ids].join(", ")}`
  );
});

test("structurally proven protections still resolve their findings", async () => {
  const shell = await securityIds(
    "proven-shell",
...
export function run(req) {
  ...
  assert.ok(
    !shell.has("FF-SEC-SHELL-001"),
    "a supported schema API plus argument separation holds"
  );

  const sql = await securityIds(
    "proven-sql",
    `export async function load(req, db) {
...
test("a same-file helper is judged by its analyzed body, not its name", async () => {
  const laundering = await securityIds(
    "helper-identity",
    `function toSafeRef(value) { return value; }
import { spawn } from "node:child_process";
export function run(req) {
  ...
  const analyzed = await securityIds(
    "helper-analyzed",
    `function toSafeRef(value) { return z.string().regex(/^[a-z0-9-]+$/).parse(value); }
import { spawn } from "node:child_process";
export function run(req) {
  return spawn("git", ["show", toSafeRef(req.query.ref)], { shell: false });
}`
  ...
  assert.ok(
    !analyzed.has("FF-SEC-SHELL-001"),
    "a helper whose body is a supported schema call is actually analyzed"
  );
});
...
function mapCase(declaration: string, extra = ""): string {
  return `${declaration}
${extra}
export function proxy(req) {
...
test("a property write on the destination map defeats the proof", async () => {
  const ids = await securityIds(
    "map-property-write",
    mapCase(
      `const DESTINATIONS = { docs: "https://docs.example.com/" };`,
      "DESTINATIONS.docs = req.query.url;"
    )
  );
...
test("a frozen map of cloud-metadata destinations is not safe", async () => {
  const ids = await securityIds(
    "map-metadata",
    mapCase(
      `const DESTINATIONS = Object.freeze({ metadata: "http://169.254.169.254/latest/meta-data/" });`
    )
  );
  assert.ok(
    ids.has("FF-SEC-SSRF-001"),
    "the metadata endpoint is the SSRF target, not the defence"
  );
});
...
test("a frozen map of loopback destinations is not safe", async () => {
  const ids = await securityIds(
    "map-loopback",
    mapCase(`const DESTINATIONS = Object.freeze({ local: "http://127.0.0.1:3000/" });`)
  );
  assert.ok(ids.has("FF-SEC-SSRF-001"));
});
...
test("passing the map to an unknown function defeats the proof", async () => {
  const ids = await securityIds(
    "map-unknown-call",
    mapCase(
      `const DESTINATIONS = Object.freeze({ docs: "https://docs.example.com/" });`,
      "mutate(DESTINATIONS);"
    )
  );
...
test("exporting or returning the map defeats the proof", async () => {
  const exported = await securityIds(
    "map-exported",
    `export const DESTINATIONS = { docs: "https://docs.example.com/" };
export function proxy(req) {
  const destination = DESTINATIONS[req.query.key];
  return ${REDIRECT_SAFE_SINK};
}`
  ...
  const returned = await securityIds(
    "map-returned",
    mapCase(
      `const DESTINATIONS = { docs: "https://docs.example.com/" };`,
      "function leak() { return DESTINATIONS; }"
    )
  );
...
test("literal destination classification refuses every internal range", () => {
  ...
  for (const value of unsafe)
    assert.equal(
      classifyDestination(value).safe,
      false,
      `${value} must not be treated as external`
    );
});
...
test("hostname destinations are accepted but recorded as DNS-dependent", () => {
  ...
  assert.equal(
    verdict.dns_dependent,
    true,
    "no resolution is performed, so DNS rebinding stays outside the proof"
  );
