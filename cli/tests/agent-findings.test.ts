...
test("ancestor findings rebase only when cited content is unchanged", async () => {
  await withTemporaryProject("agent-binding-ancestor", async (root) => {
    ...
    await writeFile(
      join(root, "route.ts"),
      "export const clinicId = request.body.clinicId;\n",
      "utf8"
    );
...
test("invalid revisions and mismatched exact snapshots are rejected", async () => {
  await withTemporaryProject("agent-binding-invalid", async (root) => {
    ...
    await assert.rejects(
      bindAgentFindings(root, [finding("not-a-revision", source)]),
      /malformed Git revision/u
    );
    await assert.rejects(
      bindAgentFindings(root, [finding("f".repeat(40), source)]),
      /nonexistent Git revision/u
    );
...
function finding(revision: string, source: string): Finding {
  return {
    id: "FF-TENANT-900",
...
async function initializeGit(root: string, source: string): Promise<string> {
