...
const UNRELATED_MIDDLEWARE: Record<string, string> = {
...
const UNRELATED_EXPORTS: Record<string, string> = {
  csrf: "verifyCsrf",
  quota: "enforceQuota",
  mime: "onlyImages",
...
for (const [concern, source] of Object.entries(UNRELATED_MIDDLEWARE)) {
  ...
  test(`a ${concern} rejection is not accepted as an authorization guard`, async () => {
    const ids = await authorizationIds(`guard-unrelated-${concern}`, {
      ...
      "routes.ts": ROUTE_SOURCE(
        `import { ${exported} } from "./middleware/unrelated.js";`,
        exported ?? ""
      )
    });
    assert.ok(
      ids.has(ROUTE),
      `a ${concern} rejection answers 401/403 without deciding authority and must not clear the route`
    );
...
const AUTHORIZATION_MIDDLEWARE: Record<string, [string, string]> = {
  role: [
    "requireEditorRole",
...
test("an unrelated middleware beside a real guard does not hide the guard", async () => {
  const ids = await authorizationIds("guard-unrelated-plus-real", {
    ...
    "routes.ts": ROUTE_SOURCE(
      `import { verifyCsrf } from "./middleware/unrelated.js";
import { ensureAdmin } from "./middleware/guard.js";`,
      "verifyCsrf, ensureAdmin"
    )
  });
...
`,
  ...
  assert.ok(
    unconditional.has(ROUTE),
    "a Forbidden error thrown from a catch block decides no authorization question"
  );
...
test("route findings are byte-identical across repeated runs", async () => {
  const files = {
    ...
    "routes.ts": ROUTE_SOURCE(
      `import { enforceQuota } from "./middleware/unrelated.js";`,
      "enforceQuota"
    )
  };
