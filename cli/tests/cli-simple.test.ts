...
test("bare update and uninstall stay scoped to hosts recorded by the installation", async () => {
  await withTemporaryProject("bare-host-scope", async (root) => {
    const initialized = await runFile(
      process.execPath,
      [cli, "init", "claude", "--root", root],
      root
    );
    assert.equal(initialized.exitCode, 0, initialized.stderr);

    const updated = await runFile(
      process.execPath,
    ...
    assert.equal((JSON.parse(removed.stdout) as { selector: string }).selector, "claude");
...
test("plain-language build creates a safe feature and continue resumes the only unfinished item", async () => {
  await withTemporaryProject("simple-build", async (root) => {
    const built = await runFile(
      process.execPath,
      [cli, "build", "add customer login", "--root", root],
      root
    );
    ...
    const state = JSON.parse(
      await readFile(join(root, ".forge", "build", "features", "customer-login.json"), "utf8")
    ) as { summary: string; phase: string };
...
test("plain-language slug collisions are deterministic and do not overwrite prior state", async () => {
  await withTemporaryProject("simple-collision", async (root) => {
    ...
    const second = await runFile(
      process.execPath,
      [cli, "build", "create customer login", "--root", root],
      root
    );
  ...
  await withTemporaryProject("simple-continue-ambiguous", async (root) => {
    for (const slug of ["first-feature", "second-feature"]) {
      const result = await runFile(
        process.execPath,
        [cli, "feature", slug, "--summary", slug, "--root", root],
        root
      );
...
test("a transparent composite audit request runs each named discipline", async () => {
  await withTemporaryProject("simple-audit-composite", async (root) => {
    const result = await runFile(
      process.execPath,
      [cli, "audit", "uploads and file storage", "--json", "--root", root],
      root
    );
    assert.notEqual(result.exitCode, 1, result.stderr);
    const parsed = JSON.parse(result.stdout) as {
      report: { scope: string; module_decisions: Array<{ module: string }> };
    };
...
test("verify exits incomplete when disappearance does not prove resolution", async () => {
  await withTemporaryProject("simple-verify-not-verified", async (root) => {
    ...
    const verify = await runFile(
      process.execPath,
      [cli, "verify", "security", "--json", "--root", root],
      root
    );
    ...
    assert.ok(
      parsed.report.findings.some((finding) =>
        finding.evidence.some((item) => item.includes("disappearance alone"))
      )
    );
  });
});
...
test("doctor gives the exact resume command for an interrupted installation", async () => {
  await withTemporaryProject("simple-doctor-interrupted", async (root) => {
    ...
    const interrupted = await runFile(
      process.execPath,
      [cli, "doctor", "--offline", "--root", root],
      root
    );
    ...
    assert.match(
      interrupted.stdout,
      /Run 'forge update all' to resume or repair the incomplete installation/u
    );

    const repaired = await runFile(
      process.execPath,
      [cli, "update", "generic", "--root", root],
      root
    );
    assert.equal(repaired.exitCode, 0, repaired.stderr);
    const healthy = await runFile(
      process.execPath,
      [cli, "doctor", "--offline", "--root", root],
      root
    );
...
test("the quickstart demo completes audit, preview, safe fix, and verification", async () => {
  await withTemporaryProject("simple-demo", async (root) => {
    ...
    const applied = await runFile(
      process.execPath,
      [cli, "fix", "frontend", "--safe", "--root", root],
      root
    );
