...
import type { CommandDefinition, GateEvidence } from "../src/types.js";
...
test("current Ship inspection evidence is fresh, file-bound, and independently verified", async () => {
  await withTemporaryProject("ship-current-evidence", async (root) => {
    ...
    const current = secrets.evidence_records.find(
      (record) => record.evidence_type === "secret-scan"
    );
    assert.ok(current);
    ...
    assert.equal(
      (await verifyEvidenceEnvelope({ root, revision: result.revision, evidence: current }))
        .verified,
      true
    );
    ...
    assert.equal(
      (await verifyEvidenceEnvelope({ root, revision: result.revision, evidence: editedClaim }))
        .verified,
      false
    );
...
test("a command that edits its bound definition cannot produce a current PASS claim", async () => {
  await withTemporaryProject("ship-command-edits-source", async (root) => {
    ...
    assert.equal(
      licenses.evidence_records.some(
