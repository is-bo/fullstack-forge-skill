  ...
  await withTemporaryProject("gate-forge-separation", async (root) => {
    ...
    for (const gateId of [
      ...
      assert.equal(
        gate.status,
        "NOT_APPLICABLE",
        `${gateId} is a Forge self-release check and must not be demanded of an application`
      );
...
test("every registry gate declares an explicit applicability class", () => {
  for (const definition of FORGE_GATE_REGISTRY)
    assert.ok(
      ["forge-self", "audited-application", "project-native"].includes(definition.applicability),
      `${definition.gate_id} must declare an applicability class`
    );
});
...
test("no registry gate is both forge-self and backed by application evidence", () => {
  for (const definition of FORGE_GATE_REGISTRY)
    if (definition.applicability === "forge-self")
      assert.equal(
        definition.evidence_sections,
        undefined,
        `${definition.gate_id} cannot be a self-check and an application evidence gate at once`
      );
});
