...
type Slug = (typeof EXPECTED)[number];
...
} satisfies Record<Slug, readonly string[]>;
...
test("canonical catalog and generated command directories match", async () => {
  ...
  assert.deepEqual(
    directoryNames.filter((name) => !EXPECTED_WORKFLOW_COMMANDS.includes(name)),
    auditDirectories
  );
  const unexpected = directoryNames.filter(
    (name) => !auditDirectories.includes(name) && !EXPECTED_WORKFLOW_COMMANDS.includes(name)
  );
...
test("explicit inspection criteria cover every module and render into canonical skills", async () => {
  ...
  for (const slug of EXPECTED) {
    ...
    assert.ok(
      moduleCriteria.every(
        (criterion) => criterion === criterion.trim() && !/[\r\n]/u.test(criterion)
      ),
      `${slug}: criteria must be normalized single lines`
    );
    ...
    for (const match of toolReferences)
      assert.ok(
        TOOL_NAMES.includes(match[1] as (typeof TOOL_NAMES)[number]),
