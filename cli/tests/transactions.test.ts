...
async function transactionFindings(name: string, source: string) {
...
async function transactionIds(name: string, source: string) {
...
test("related writes sharing an entity identifier without a boundary are reported", async () => {
  const ids = await transactionIds(
    "txn-related-missing",
...
test("two genuinely independent writes stay clean", async () => {
  const ids = await transactionIds(
    "txn-independent",
    `export async function unrelated(req, res) {
...
test("an unresolved custom wrapper is NOT_VERIFIED rather than pass or fail", async () => {
  const findings = await transactionFindings(
    "txn-unresolved-wrapper",
...
test("financial writes justify a higher severity than ordinary parent/child writes", async () => {
  const ledger = await transactionFindings(
    "txn-severity-ledger",
...
`
  ...
  assert.ok(
    financial.every((finding) => finding.severity === "HIGH" || finding.severity === "CRITICAL"),
    `financial severity was ${financial.map((finding) => finding.severity).join(", ")}`
  );
});
...
`
  ...
  assert.ok(
    ids.has(MISSING) || ids.has(UNRESOLVED),
    `file-global BEGIN/COMMIT markers proved a boundary that does not exist; got ${[...ids].join(", ") || "nothing"}`
  );
});
...
`
  ...
  assert.ok(
    ids.has(MISSING) || ids.has(UNRESOLVED),
    `a COMMIT between the BEGIN and the write must end the boundary; got ${[...ids].join(", ") || "nothing"}`
  );
});
...
`
  ...
  assert.equal(
    ids.size,
    0,
    `a rolled-back transaction applies all writes or none: ${[...ids].join(", ")}`
  );
});
...
`
  ...
  assert.ok(
    ids.has(UNRESOLVED),
    `a caller-supplied handle is unresolved, not proven; got ${[...ids].join(", ") || "nothing"}`
  );
  assert.ok(
    !ids.has(MISSING),
    "a caller-supplied handle must not become a confident failure either"
  );
});
...
test("a transaction option threaded through a property path is unresolved, not absent", async () => {
  const findings = await transactionFindings(
    "txn-option-path",
    `export async function checkout(ctx, order) {
...
`
  ...
  assert.ok(
    !ids.has(MISSING),
    "an explicitly threaded handle must not be reported as no transaction at all"
  );
});

test("a payload column named like a handle does not downgrade a real defect", async () => {
  const findings = await transactionFindings(
    "txn-payload-handle-name",
    `export async function checkout(order, session) {
...
`
  ...
  assert.ok(
    ids.has(MISSING),
    `expected a confident ${MISSING}, got ${[...ids].join(", ") || "nothing"}`
  );
...
`
  ...
  assert.ok(
    !ids.has(MISSING),
    "an unresolvable transaction factory must not produce a confident failure"
  );
});

test("an ordinary client from an unresolvable factory remains a confident failure", async () => {
  const findings = await transactionFindings(
    "txn-ordinary-client-origin",
    `export async function checkout(order) {
...
`
  ...
  assert.ok(
    ids.has(MISSING),
    `the commonest genuine defect shape must stay confident; got ${[...ids].join(", ") || "nothing"}`
  );
...
`
  ...
  assert.ok(
    !ids.has(MISSING),
    "savepoint versus independent-connection nesting is vendor-specific and not decidable here"
  );
});

test("two sibling transactions over related writes remain a confident split", async () => {
  const findings = await transactionFindings(
    "txn-sibling-transactions",
    `export async function checkout(order) {
  await prisma.$transaction(async (tx) => {
    await tx.order.create({ data: { id: order.id } });
  });
...
test("a workflow whose every write happens inside a helper is still analysed", async () => {
  const ids = await transactionIds(
    "txn-helper-only-workflow",
    `async function recordEntry(orderId, amount) {
...
`
  ...
  assert.ok(
    ids.has(MISSING),
    `two calls to one helper are two related writes; got ${[...ids].join(", ") || "nothing"}`
  );
});

test("a helper writing through its own receiver under a caller transaction is NOT_VERIFIED", async () => {
  const findings = await transactionFindings(
    "txn-helper-own-receiver",
    `async function writeLedger(orderId, amount) {
  await prisma.ledgerEntry.create({ data: { orderId: orderId, amount: amount } });
}
export async function checkout(order) {
  await prisma.$transaction(async (tx) => {
...
test("writes sharing only an ownership or tenancy identifier are not related", async () => {
  const ownership = await transactionIds(
    "txn-owner-scope",
    `export async function cleanup(req) {
...
`
  ...
  assert.equal(
    ownership.size,
    0,
    `a shared owner is not one invariant: ${[...ownership].join(", ")}`
  );
  const tenancy = await transactionIds(
    "txn-tenant-scope",
...
`
  ...
  assert.equal(
    ids.size,
    0,
    `writes in separate functions are separate workflows: ${[...ids].join(", ")}`
  );
});
...
`
  ...
  assert.ok(
    ids.has(MISSING),
    `a retry helper is not a transaction; got ${[...ids].join(", ") || "nothing"}`
  );
});

test("severity follows the demonstrated domain and every unresolved finding is NOT_VERIFIED", async () => {
  const financial = await transactionFindings(
    "txn-severity-financial",
...
`
  ...
  assert.ok(
    confirmed.every((finding) => finding.severity === "CRITICAL"),
    `a financial amount write is CRITICAL, got ${confirmed.map((entry) => entry.severity).join(", ")}`
  );
  const inventory = await transactionFindings(
    "txn-severity-inventory",
    `export async function release(req) {
...
`
  ...
  assert.ok(
    stock.every((finding) => finding.severity === "HIGH"),
    `inventory pairs are documented as HIGH, got ${stock.map((entry) => entry.severity).join(", ")}`
  );
});

test("a row filter naming an amount column does not escalate severity", async () => {
  const findings = await transactionFindings(
    "txn-severity-filter-only",
    `export async function annotate(req) {
...
`
  ...
  assert.ok(
    reported.every((finding) => finding.severity !== "CRITICAL"),
    `a filter column is not an amount mutation, got ${reported.map((entry) => entry.severity).join(", ")}`
  );
});

test("every unresolved-boundary finding carries LOW confidence and NOT_VERIFIED status", async () => {
  const findings = await transactionFindings(
    "txn-unresolved-contract",
    `export async function settle(req) {
  await mysteryUnit(async (handle) => {
    await handle.invoice.update({ where: { id: req.body.id }, data: { status: "PAID" } });
    await handle.receipt.create({ data: { invoiceId: req.body.id, amount: req.body.total } });
  });
...
`;
  ...
  assert.deepEqual(
    first.map((finding) => `${finding.instance_id ?? finding.id}:${finding.severity}`).sort(),
    second.map((finding) => `${finding.instance_id ?? finding.id}:${finding.severity}`).sort()
  );
});
