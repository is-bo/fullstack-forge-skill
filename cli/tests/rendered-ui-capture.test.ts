...
type FakeSpec = {
  ...
  supportsStructuralAdapter?: boolean;
  structural?: Array<{
...
test("fixed structural observations record responsive, keyboard-accessible dialog, and custom-control facts", async () => {
  ...
  if (tablet === undefined) throw new Error("tablet observation unexpectedly missing");
  ...
  if (mobile === undefined) throw new Error("mobile observation unexpectedly missing");
  assert.equal(mobile.status, "FAIL");
...
test("offline mode blocks a redirect to a public hostname", async () => {
  ...
  assert.deepEqual(
    reachedHosts.filter((host) => host !== "127.0.0.1"),
    [],
