...
async function writeRunnableProject(
  root: string,
  scripts: readonly string[] = ["test", "lint", "check:security-review"]
): Promise<void> {
  await writeProjectScripts(root, Object.fromEntries(scripts.map((name) => [name, PASS_SCRIPT])));
}

async function writeProjectScripts(
  root: string,
...
async function plantWorkingBrowserDriver(root: string): Promise<void> {
...
async function planAndCheck(root: string, slug: string): Promise<{ code: number; out: string }> {
  await captureRun([
    "feature",
    slug,
    "plan",
...
test("light tier completes in two invocations through registered behavior and code producers", async () => {
  await withTemporaryProject("build-light", async (root) => {
    ...
    for (const criterion of [
      ...
      assert.equal(
        afterStart.evidence.find((entry) => entry.criterion === criterion)?.status,
        "PASS",
        criterion
      );
...
test("Build command output is redacted before sealing and remains verifiable after reload", async () => {
  await withTemporaryProject("build-output-redaction", async (root) => {
    ...
    const raw = await readFile(
      join(root, ".forge", "build", "features", "redacted-output.json"),
      "utf8"
    );
...
test("done reports the exact non-waivable high-tier security gate", async () => {
  await withTemporaryProject("build-done-refuse", async (root) => {
    ...
    const planted = JSON.parse(await readFile(featurePath, "utf8")) as {
      gate_plan: { gates: unknown[]; required_criteria: string[] };
    };
...
test("accept-risk is refused for a high-tier required security control", async () => {
  await withTemporaryProject("build-accept-refuse", async (root) => {
    ...
    const forgedGate = planted.gate_plan.gates.find((gate) =>
      gate.criteria.includes("discipline:auth")
    );
...
test("operational risk acceptance requires an actor, stays distinct from PASS, and expires on change", async () => {
  await withTemporaryProject("build-accept-operational", async (root) => {
    ...
    assert.equal(
      accepted.evidence.find((entry) => entry.criterion === "discipline:observability")?.status,
      "NOT_VERIFIED"
    );
    ...
    assert.equal(
      (JSON.parse(staleStatus.out) as { feature: { phase: string } }).feature.phase,
      "check"
    );
...
test("the repair cap blocks a feature whose supported-pattern failure signature recurs", async () => {
  await withTemporaryProject("build-repair", async (root) => {
    ...
    assert.equal(
      feature.evidence.find((entry) => entry.criterion === "supported-static-patterns")?.status,
      "FAIL"
    );
    assert.equal(
      feature.repair_counters.find((counter) => counter.criterion === "supported-static-patterns")
        ?.count,
      1
    );
...
test("resume lists unfinished features and points at the most recent", async () => {
  await withTemporaryProject("build-resume", async (root) => {
    ...
    const plantedIndex = JSON.parse(await readFile(projectPath, "utf8")) as {
      features: unknown[];
    };
    ...
    const rebuiltIndex = JSON.parse(await readFile(projectPath, "utf8")) as {
...
test("missingForDone accepts only positive criteria verified by the current caller", () => {
  const feature: BuildFeature = newFeature("f", "standard", "s");
  feature.phase = "check";
...
test("missingForDone binds risk acceptance to the current gate policy and accountable actor", () => {
  ...
  assert.ok(
    missingForDone(feature).some((item) => item.includes("discipline:observability")),
    "a weaker advisory acceptance must not satisfy an operational-human gate"
  );
...
test("registered producers can complete all twelve material discipline paths", async () => {
  await withTemporaryProject("build-discipline-e2e", async (root) => {
    ...
    const scriptFor: Record<(typeof disciplines)[number], string> = {
      auth: "test:auth",
    ...
    assert.ok(feature);
    for (const discipline of disciplines) {
      const record: CriterionEvidence | undefined = feature.evidence.find(
        (entry) => entry.criterion === `discipline:${discipline}`
      );
...
test("a high-tier authentication and authorization feature reaches done only through real evidence", async () => {
  await withTemporaryProject("build-high-auth-e2e", async (root) => {
    ...
    for (const criterion of [
      ...
      assert.equal(
        feature.evidence.find((entry) => entry.criterion === criterion)?.status,
        "PASS",
        criterion
      );
...
test("a high-tier UI feature reaches done with a bound eight-state responsive runtime matrix", async () => {
  await withTemporaryProject("build-high-ui-e2e", async (root) => {
    ...
    await writeFile(
      join(root, ".forge", "build", "DESIGN.md"),
    ...
    assert.equal(
      feature.evidence.find((entry) => entry.criterion === "design-direction")?.status,
      "PASS"
    );
