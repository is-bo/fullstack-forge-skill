...
import type { AnalyzerCoverage } from "../src/types.js";
...
): Promise<{ coverage: AnalyzerCoverage[]; markdown: string }> {
  await writeFile(
    join(root, "package.json"),
    JSON.stringify({ name: "adapter-project", private: true, dependencies }),
    "utf8"
  );
    ...
    await t.test(framework, async () => {
      await withTemporaryProject("adapter-" + framework, async (root) => {
        ...
        const coverage = result.coverage.find(
          (entry) => entry.language === "Python" && entry.framework === framework
        );
        assert.ok(coverage);
