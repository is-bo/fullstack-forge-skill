...
test("every evidence class carries a declared activation weight", () => {
  ...
  for (const activating of ["implementation", "route", "schema"] as const)
...
test("real JSX structure proves frontend use when a dependency declaration alone cannot", async () => {
  await withTemporaryProject("discovery-jsx-frontend", async (root) => {
    await writeProject(root, {
      ...
      "Link.tsx":
        'export const Link = () => <a href="https://example.com" target="_blank">Docs</a>;\n'
    });
...
test("concrete deployment, Terraform, robots, and sitemap shapes activate only in active trees", async () => {
  await withTemporaryProject("evidence-concrete-file-shapes", async (root) => {
    ...
    for (const capability of ["deployment", "infrastructure", "public-web"]) {
      ...
      assert.ok(
        assessment.evidence.some((item) => item.activation_weight >= ACTIVATION_THRESHOLD),
        capability
      );
    }
    const deployment = find(assessments, "deployment");
    assert.equal(
      deployment.evidence.find((item) => item.path === "docs/Dockerfile")?.activation_weight,
      0
    );
    assert.equal(
      deployment.evidence.find((item) => item.path === "tests/Dockerfile")?.activation_weight,
      0
    );
    const infrastructure = find(assessments, "infrastructure");
    assert.equal(
      infrastructure.evidence.find((item) => item.path === "examples/legacy/main.tf")
        ?.activation_weight,
      0
    );
    const publicWeb = find(assessments, "public-web");
    assert.equal(
      publicWeb.evidence.find((item) => item.path === "fixtures/site/robots.txt")
        ?.activation_weight,
      0
    );
    assert.equal(
      publicWeb.evidence.find((item) => item.path === "skills/site/sitemap.xml")?.activation_weight,
      0
    );
  });
});
...
test("an unused Stripe dependency remains UNKNOWN and does not auto-select payments", async () => {
  await withTemporaryProject("evidence-unused-stripe", async (root) => {
    ...
    assert.deepEqual(
      [...new Set(payments.evidence.map((item) => item.evidence_class))],
      ["manifest"]
    );
...
test("an undeclared archived package cannot aggregate React capability or framework evidence", async () => {
  await withTemporaryProject("evidence-undeclared-archive", async (root) => {
    await writeProject(root, {
      ...
      "archive/old-ui/src/App.tsx":
        "ReactDOM.createRoot(document.body).render(<main>Archived</main>);\n"
    });
    ...
    assert.equal(
      (profile.capability_assessments ?? []).some((item) => item.workspace === "archive/old-ui"),
      false
    );
    ...
    assert.ok(
      profile.workspaces.some(
        (workspace) => workspace.root === "archive/old-ui" && workspace.type === "nested-package"
      )
    );
    const decision = decideModules({
      candidates: ["frontend"],
...
`,
      ".agents/skills/forge-payments/SKILL.md":
        "Call stripe.checkout.sessions.create and inspect payment webhooks.\n",
...
test("a real session boundary auto-selects authentication", async () => {
  ...
  res.json({ userId: session.user.id });
    ...
    const profile = await discoverProject(root);
    const identity = (profile.risk_evidence ?? []).find(
      (item) => item.risk === "identity-or-session-boundary"
    );
    assert.ok(identity);
...
`
    ...
    const profile = await discoverProject(root);
    assert.deepEqual(
      profile.routes.map((route) => route.name),
      ["GET /health"]
    );
...
test("risk signatures cover payment webhooks, uploads, and tenant background jobs", async () => {
  await withTemporaryProject("risk-signature-matrix", async (root) => {
    await writeProject(root, {
      ...
      "jobs/reminders.ts":
        "export const reminderJob = defineJob(async (clinicId) => db.patient.findMany({ where: { clinicId } }));"
    });
    const profile = await discoverProject(root);
    const modules = new Set((profile.risk_evidence ?? []).flatMap((evidence) => evidence.modules));
    for (const module of [
...
test("a static site has no authorization risk surface in the bounded scope", async () => {
  await withTemporaryProject("risk-static-site", async (root) => {
    ...
    const profile = await discoverProject(root);
    assert.ok(
      !(profile.risk_evidence ?? []).some((item) => item.modules.includes("authorization"))
    );
  });
});
...
model Appointment {
  ...
  clinicId String
      ...
      "routes/patients.ts":
        "export const list = (req) => prisma.patient.findMany({ where: { clinicId: req.session.user.clinicId } });"
    });
    const profile = await discoverProject(root);
