...
async function uploadFindings(name: string, source: string) {
...
test("content-type validation delegated to an imported helper is NOT_VERIFIED, not a failure", async () => {
  const findings = await uploadFindings(
    "upload-delegated-validation",
...
`
  ...
  assert.equal(
    mime[0]?.status,
    "NOT_VERIFIED",
    "validation delegated outside the file must not be a confident failure"
  );
  assert.match(
    mime[0].evidence.join(" "),
    /assertAllowedType/u,
    "evidence must name the delegate that could not be resolved"
  );
});

test("client MIME trusted with no validation anywhere remains a failure", async () => {
  const findings = await uploadFindings(
    "upload-unvalidated-mime",
    `const upload = multer({ limits: { fileSize: 5_000_000, files: 1 } });
app.post("/files", upload.single("doc"), async (req, res) => {
...
`
  ...
  assert.equal(
    mime[0]?.status,
    "FAIL",
    "trusting client MIME with no validation at all is a proven defect"
  );
});

test("in-file signature validation keeps the upload path clean", async () => {
  const findings = await uploadFindings(
    "upload-infile-signature",
...
`
  ...
  assert.equal(
    findings.filter((finding) => finding.id === MIME).length,
    0,
    "proven in-file signature validation must not report"
  );
});
