...
const GENERATED_OR_CACHE = new Set<string>([
...
const TEXT_NAMES =
  /^(?:Dockerfile|Jenkinsfile|Makefile|Procfile|Rakefile|Gemfile|go\.mod|go\.sum|gradlew|package-lock\.json|pnpm-lock\.yaml|yarn\.lock|\.env(?:\..+)?)$/u;
...
export type InventoryEvidenceClass =
  | "configuration"
  | "documentation"
  | "example"
  | "fixture"
...
export type RepositoryInventoryDiagnostics = {
  ...
  excluded_paths: Array<{ path: string; category: string; pattern?: string }>;
...
type CandidateDiscovery = {
  ...
  gitRoot?: string;
  gitFailure?: string;
  fallbackExcluded: Array<{ path: string; category: string; pattern?: string }>;
...
export async function inventoryRepository(
  ...
  let binarySkipped = 0;
  let ignoredExcluded = discovery.fallbackExcluded.filter(
    (item) => item.category === "gitignore"
  ).length;
  let generatedExcluded = discovery.fallbackExcluded.filter(
    (item) => item.category === "generated"
  ).length;
  let defaultExcluded = discovery.fallbackExcluded.filter((item) =>
    item.category.startsWith("default:")
  ).length;
  ...
  const neutralRepresentatives = new Map<string, number>();
  if (
    discovery.fallbackExcluded.some(
      (item) => item.category === "user" && !isNeutralEvidence(classifyInventoryPath(item.path))
    )
  )
  ...
  for (const candidate of discovery.candidates) {
    ...
    if (userMatch !== undefined) {
      ...
      userExcluded += 1;
      if (!isNeutralEvidence(evidenceClass)) requiredEvidenceExcluded = true;
      continue;
    }
    const gitignoreMatch =
      candidate.origin === "fallback" ? matchingPattern(safePath, gitignore.patterns) : undefined;
    ...
    if (defaultDecision === "exclude" && defaultCategory !== undefined) {
      excludedPaths.push({
        path: safePath,
        category:
          defaultCategory.category === "generated-build-output" ||
    ...
    try {
      ...
      if (!isNeutralEvidence(evidenceClass)) partialReason ??= "relevant-file-read-failed";
    }
  }

  if (requiredEvidenceExcluded) partialReason ??= "user-exclusions-affect-required-evidence";
  const status: InventoryStatus = partialReason === undefined ? "COMPLETE" : "PARTIAL";
  const diagnostics: RepositoryInventoryDiagnostics = {
    status,
...
export function parseInspectionBudget(value: string): number {
  const match = /^([1-9][0-9]*)(?:\s*(B|KiB|MiB))?$/u.exec(value.trim());
  if (match === null)
    throw new Error(
      `Option '--inspection-budget' requires a positive byte, KiB, or MiB value, got '${value}'`
    );
...
export function validateExclusionPattern(value: string, source = "--exclude"): string {
  ...
  if (
    ...
    throw new Error(
      `${source} pattern must be repository-relative and cannot escape the root: ${value}`
    );
...
export function classifyInventoryPath(path: string): InventoryEvidenceClass {
  ...
  if (
    lowerDirectories.some(
      (segment, index) =>
        GENERATED_OR_CACHE.has(segment) && (!GENERIC_OUTPUT_DIRECTORIES.has(segment) || index === 0)
    ) ||
  ...
  if (lowerDirectories.some((segment) => FIXTURE_SEGMENTS.has(segment))) return "fixture";
  if (
    lowerDirectories.some((segment) => TEST_SEGMENTS.has(segment)) ||
    /\.(?:spec|test)\.[^.]+$/u.test(name)
  )
    return "test";
  if (lowerDirectories.some((segment) => EXAMPLE_SEGMENTS.has(segment))) return "example";
  if (isManifestName(name)) return "manifest";
  if (
    name.startsWith(".env.") ||
  ...
  if (TEXT_EXTENSIONS.has(extensionOf(normalized))) return "production";
  return "unknown";
}

export function renderInventoryIncomplete(diagnostics: RepositoryInventoryDiagnostics): string {
  const contributors =
    diagnostics.largest_contributing_directories.length === 0
...
async function discoverCandidates(
  ...
  options: {
    maxEntries: number;
    maxDepth: number;
    policyPatterns: CompiledPattern[];
    userPatterns: CompiledPattern[];
    gitignorePatterns: CompiledPattern[];
    applyDefaultExclusions: boolean;
  }
...
async function fallbackCandidates(
  ...
  const excluded: Array<{ path: string; category: string; pattern?: string }> = [];
  let partialReason: string | undefined;
  async function visit(directory: string, depth: number): Promise<void> {
    if (partialReason === "inventory-entry-limit-exceeded") return;
...
async function loadIgnoreFile(
  ...
  name: string,
  strict: boolean
): Promise<{ patterns: CompiledPattern[] }> {
  const path = join(root, name);
  let text: string;
  try {
    const info = await stat(path);
    if (!info.isFile()) return { patterns: [] };
    if (info.size > 256 * 1024) throw new Error(`${name} exceeds the 262144-byte policy limit`);
    text = await readFile(path, "utf8");
  } catch (error) {
    if (errorCode(error) === "ENOENT") return { patterns: [] };
  ...
  for (const [index, raw] of text.split(/\r?\n/u).entries()) {
    ...
    if (value.startsWith("!")) {
      if (strict)
        throw new Error(
          `${name}:${index + 1}: negation patterns are not supported; use a narrower positive exclusion`
        );
      continue;
    }
    try {
...
async function binaryProbe(path: string): Promise<{ binary: boolean; bytesRead: number }> {
  ...
  try {
    ...
    if (bytes.includes(0)) return { binary: true, bytesRead: result.bytesRead };
    let controls = 0;
    for (const byte of bytes) if (byte < 0x09 || (byte > 0x0d && byte < 0x20)) controls += 1;
    return {
...
async function canonicalInventoryRoot(path: string): Promise<string> {
...
function normalizeSafeRelative(path: string): string {
...
function isInsideInventoryRoot(root: string, candidate: string): boolean {
  const path = relative(resolve(root), resolve(candidate));
  return path === "" || (!path.startsWith(`..${sep}`) && path !== ".." && !isAbsolute(path));
}

function isNeutralEvidence(value: InventoryEvidenceClass): boolean {
  return ["documentation", "example", "fixture", "generated", "test"].includes(value);
}

function isManifestName(name: string): boolean {
  return /^(?:cargo\.toml|gemfile|go\.mod|package\.json|pom\.xml|pyproject\.toml|requirements[^/]*\.txt|settings\.gradle(?:\.kts)?|build\.gradle(?:\.kts)?)$/u.test(
    name
  );
}
...
function addDirectoryBytes(target: Map<string, number>, path: string, bytes: number): void {
...
function compareContributors(a: InventoryContributor, b: InventoryContributor): number {
  return b.bytes - a.bytes || compareText(a.path, b.path);
}

function compactFailure(result: { exitCode: number; stdout: string; stderr: string }): string {
...
function validateBudget(name: string, value: number, maximum: number): void {
