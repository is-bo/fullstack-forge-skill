...
const BASE_FRONTEND =
  /\b(?:front[ -]?end|react|next(?:\.js)?|vue|svelte|component|page|layout|browser|hydration|client state|responsive|mobile[- ]first|form|table|chart|dashboard|react native|expo)\b/iu;
const BASE_UI =
  /\b(?:ui|interface|component|visual design|redesign|styling|spacing|typography|colou?r|design system|visual consistency|dark mode|icon|motion|landing page|dashboard|polish)\b/iu;
const BASE_UX =
  /\b(?:ux|user experience|user flow|usability|navigation|form|booking|onboarding|task completion|empty state|feedback|recovery|information architecture|conversion|friction)\b/iu;
...
const RULES: readonly RoutingRule[] = [
  ...
  {
    pattern:
      /\b(?:dashboard|analytics|chart|visualization|visualisation|admin table|data table|inventory|point of sale|pos)\b/iu,
    modules: ["ui", "ux"],
  ...
  {
    pattern:
      /\b(?:healthcare|medical|payment|authentication|authorization|permission|destructive|financial|sensitive)\b/iu,
    modules: ["security"],
 ...
 */
export function routeFrontendRequest(request: string, area?: FrontendArea): FrontendRoute {
