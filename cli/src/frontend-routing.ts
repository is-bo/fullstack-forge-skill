...
export type CompletionApplicability = Record<
  CompletionConcern,
  { status: "REQUIRED" | "NOT_APPLICABLE"; reason: string }
>;
...
type RoutingRule = {
  ...
  references?: readonly FrontendReferenceId[];
  reason: string;
};

const STRONG_FRONTEND =
  /\b(?:front[ -]?end|react(?: native)?|next(?:\.js)?|vue|svelte|jsx|tsx|html interface|browser|css|responsive(?: layout| interface)?|mobile[- ]first|hydration|client component|client state|landing page|mobile interface|visual redesign|expo|button)\b/iu;
const STRONG_UI =
  /\b(?:ui|user interface|html interface|visual design|visual redesign|redesign|styling|spacing|typography|colou?r|design system|visual consistency|dark mode|iconography|landing page|visual polish|button|accessibility)\b/iu;
const STRONG_UX =
  /\b(?:ux|user experience|user flow|usability|navigation|onboarding|task completion|empty state|feedback|information architecture|conversion|friction)\b/iu;
const AMBIGUOUS_FRONTEND =
...
const RULES: readonly RoutingRule[] = [
  ...
  },
  {
    ...
    reason:
      "New or substantially redesigned interface work needs product, visual, system, and responsive decisions."
  },
  {
    ...
    reason:
      "React or Next.js evidence needs framework, composition, and measured performance guidance."
  },
  {
  ...
  {
    pattern:
      /\b(?:performance|bundle|waterfall|hydration|render|slow|latency|core web vitals|large list)\b/iu,
    modules: ["performance"],
  ...
  {
    pattern:
      /\b(?:forms?|inputs?|validation|booking|appointment|checkout|filters?|bulk actions?|data entry)\b/iu,
    modules: ["ux"],
  ...
  {
    pattern:
      /\b(?:dashboard|analytics|chart|visualization|visualisation|table|inventory|point of sale|pos)\b/iu,
    modules: ["ui", "ux"],
    references: ["dashboards-and-data-visualization", "responsive-layout"],
    reason:
      "Dense data interfaces need task-led hierarchy, accessible alternatives, and narrow-screen behavior."
  },
  {
  ...
  },
  {
    ...
    reason:
      "Native mobile work needs platform, unreliable-network, motion, and measured list guidance."
  },
  {
  ...
  {
    pattern:
      /\b(?:arabic|rtl|right[- ]to[- ]left|french|locale|localization|localisation|translation|internationalization|internationalisation)\b/iu,
    modules: ["i18n"],
    references: ["responsive-layout"],
  ...
  {
    pattern:
      /\b(?:admin(?:istrator)?s?|authorization|permission|protected action|role|ownership)\b/iu,
    modules: ["authorization", "security"],
    reason: "Protected interface actions need authorization at the final server-side sink."
  },
  {
    pattern:
      /\b(?:delete|destructive|remove patient|healthcare|medical|payment|financial|sensitive|personal data)\b/iu,
  ...
  {
    pattern:
      /\b(?:loading|empty|error|failure|failed|success|permission|partial|offline) state\b/iu,
    modules: ["ux"],
 ...
 */
export function routeFrontendRequest(
  request: string,
  area?: FrontendArea,
  evidence: FrontendRoutingEvidence = {}
): FrontendRoute {
  const text = request.trim();
  const explicit = area !== undefined;
  const affectedPaths = evidence.affectedPaths ?? [];
  const frameworkEvidence = (evidence.frameworks ?? []).some((value) =>
    /^(?:react|next(?:\.js)?|vue|svelte|react native|expo)$/iu.test(value.trim())
  );
  ...
  const profileEvidence =
    ...
    (evidence.capabilities ?? []).some((value) =>
      /^(?:frontend|browser|ui|user-interface)$/iu.test(value.trim())
    );
  ...
  if (ui) modules.add("ui");
  if (ux) modules.add("ux");
  if (!frontend && !ui && !ux) return inactiveRoute();
  ...
  if (explicit) reasons.push(`Explicit ${area} selection overrides natural-language routing.`);
  if (repositorySupport)
    reasons.push(
      "Repository profile, framework, capability, or affected-path evidence supports interface routing."
    );

  for (const rule of RULES) {
    if (!rule.pattern.test(text)) continue;
    for (const module of rule.modules ?? []) modules.add(module);
    for (const reference of rule.references ?? []) references.add(reference);
    reasons.push(rule.reason);
  }

  if (ui && references.size === 1) references.add("design-system");
  if (ux && !references.has("product-and-ux")) references.add("product-and-ux");
  if (frontend && references.size === 1) references.add("component-architecture");

  const highRisk = modules.has("security");
  const isolatedVisual =
    /\b(?:button spacing|spacing|typography|colou?r|css token|style token)\b/iu.test(text) &&
    !/\b(?:data|permission|admin|delete|security|performance|form|network|state|workflow)\b/iu.test(
      text
    );
  const small =
    /\b(?:small|single|one)[ -](?:component|style|styling|css|token)\b/iu.test(text) ||
    isolatedVisual;
  const workflow: FrontendWorkflow = /\bverify\b/iu.test(text)
    ? "verify"
...
export function assessCompletionApplicability(
  request: string,
  route: FrontendRoute = routeFrontendRequest(request)
): CompletionApplicability {
  const text = request.trim();
  const has = (module: ModuleSlug) => route.modules.includes(module);
  const workflowStates =
    route.active &&
    /\b(?:loading|empty|error|failure|failed|success|permission|partial|offline|submit|save|delete|notify|network|form)\b/iu.test(
      text
    );
  const runtimeRendered =
    route.active &&
    /\b(?:visual|ui|interface|browser|css|responsive|layout|spacing|typography|colou?r|button|render|hydration)\b/iu.test(
      text
    );

  return {
    "authentication-authorization": decision(
      has("auth") || has("authorization"),
      "The selected workflow changes identity, permissions, or a protected action.",
      "No identity, permission, tenant, or protected-action boundary was selected."
    ),
    database: decision(
      has("database") || has("queries"),
      "The selected workflow reads, writes, filters, sorts, paginates, or deletes persisted data.",
      "The selected interface change does not affect persisted-data behavior."
    ),
    "workflow-states": decision(
      workflowStates,
      "The affected workflow can reach loading, failure, permission, partial, offline, or outcome states.",
      "No reachable asynchronous or outcome state is introduced by the selected change."
    ),
    accessibility: decision(
      route.active,
      "Every human-facing change requires proportionate accessibility evidence.",
      "No human-facing interface boundary was selected."
    ),
    security: decision(
      has("security"),
      "The selected workflow changes a trust boundary or handles sensitive input or data.",
      "No sensitive input, identity, permission, secret, payment, upload, or personal-data boundary was selected."
    ),
    performance: decision(
      has("performance"),
      "The selected workflow changes performance-sensitive behavior or makes a performance claim.",
      "No performance-sensitive behavior or claim was selected."
    ),
    "runtime-rendered": decision(
      runtimeRendered,
      "Correctness depends on observed runtime or rendered interface behavior.",
      "The selected concern can be established without rendered or runtime behavior."
    )
  };
}

export function normalizeFrontendWorkflow(area: FrontendArea, mode?: string): FrontendWorkflow {
  if (mode === undefined || mode === "") return "audit";
  if (mode === "review") return "audit";
  if (mode === "improve") return "fix";
  if (["build", "audit", "fix", "verify"].includes(mode)) return mode as FrontendWorkflow;
  throw new Error(
    `Unknown ${area} workflow '${mode}'. Expected build, audit, fix, verify${area === "ui" || area === "ux" ? ", or review" : ""}${area === "ux" ? ", or improve" : ""}.`
  );
}

function inactiveRoute(): FrontendRoute {
  return {
    active: false,
    modules: [],
    references: [],
