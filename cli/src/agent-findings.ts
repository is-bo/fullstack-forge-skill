...
import type { Finding, FindingBindingState } from "./types.js";
import { resolveInside, runFile, sha256, workingTreeRevision } from "./utils.js";

export async function bindAgentFindings(root: string, findings: Finding[]): Promise<Finding[]> {
...
async function bindOne(
  ...
  isGit: boolean
): Promise<{ state: FindingBindingState; reason: string }> {
  const snapshots = finding.evidence_snapshot ?? [];
  const exact = finding.revision === currentRevision;
  if (!isGit) {
    if (!exact)
      throw new Error(
        `Finding ${finding.id} revision '${finding.revision ?? "missing"}' does not match non-Git content revision '${currentRevision}'.`
      );
  ...
  if (requested === undefined)
    throw new Error(
      `Finding ${finding.id} has malformed Git revision '${finding.revision ?? "missing"}'.`
    );
...
function gitCommitFromRevision(revision: string | undefined): string | undefined {
...
async function verifySnapshots(
  root: string,
  snapshots: NonNullable<Finding["evidence_snapshot"]>,
  ...
  for (const snapshot of snapshots) {
    ...
    if (historicalRevision === undefined) {
      if (currentHash !== snapshot.sha256)
        throw new Error(
          `Evidence snapshot hash mismatch for '${snapshot.path}' at the exact revision.`
        );
      verifyExcerpt(snapshot, current, "current");
      continue;
    }
    const historical = await runFile(
      "git",
      ["show", `${historicalRevision}:${snapshot.path}`],
      root,
      10_000
    );
    if (historical.exitCode !== 0)
      throw new Error(
        `Evidence snapshot '${snapshot.path}' does not exist at revision '${historicalRevision}'.`
      );
    if (sha256(historical.stdout) !== snapshot.sha256)
      throw new Error(
        `Evidence snapshot hash for '${snapshot.path}' does not match revision '${historicalRevision}'.`
      );
    verifyExcerpt(snapshot, historical.stdout, historicalRevision);
    if (currentHash !== snapshot.sha256) stale = true;
    else verifyExcerpt(snapshot, current, "current");
  }
  void exact;
...
function verifyExcerpt(
  snapshot: NonNullable<Finding["evidence_snapshot"]>[number],
  content: string,
  label: string
): void {
  if (snapshot.excerpt_hash === undefined || snapshot.line === undefined) return;
  const line = content.split(/\r?\n/u)[snapshot.line - 1] ?? "";
  if (sha256(line) !== snapshot.excerpt_hash)
    throw new Error(
      `Evidence excerpt mismatch for '${snapshot.path}:${snapshot.line}' in ${label}.`
    );
}

export function reconcileFindings(previous: Finding[], incoming: Finding[]): Finding[] {
  ...
  for (const finding of current) {
    if (
      ...
      !["agent-reviewed-source", "agent-rendered-review", "agent-runtime-verification"].includes(
        finding.producer ?? ""
      )
    )
      continue;
...
function sameScope(left: Finding, right: Finding): boolean {
  if (left.location.length === 0) return true;
  const rightPaths = new Set(right.location.map((location) => location.path));
  return (
    left.location.some((location) => rightPaths.has(location.path)) ||
