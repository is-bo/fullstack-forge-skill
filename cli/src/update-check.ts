...
export function parseReleaseTags(output: string): string[] {
  const versions = new Map<string, Version>();
  for (const line of output.split(/\r?\n/u)) {
    const match =
      /^[a-f0-9]{40,64}\s+refs\/tags\/v((?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*))$/u.exec(
        line.trim()
      );
...
export async function checkUpdateAvailability(
  ...
  const result = await runner(
    "git",
    ["ls-remote", "--tags", "--refs", UPSTREAM_GIT_URL, "refs/tags/v*"],
    root,
    10_000
  );
