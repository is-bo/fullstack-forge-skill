...
const GREENFIELD_FRONTEND_DEPENDENCIES = new Set<ModuleSlug>(["ui", "ux", "accessibility"]);
 ...
 */
export function explicitGreenfieldDependenciesFor(
  profile: ProjectProfile,
  modules: readonly ModuleSlug[]
): ReadonlySet<ModuleSlug> {
  if (!modules.includes("frontend") || hasProvenFrontendEvidence(profile)) return new Set();
...
export function compositionEvidenceFor(
  ...
  if (
    hasProvenFrontendEvidence(profile) ||
    [...requestedModules].some((module) =>
      (["accessibility", "frontend", "ui", "ux"] as ModuleSlug[]).includes(module)
    )
  )
    riskSurfaces.add("frontend");
  if (
    requestedModules.has("api") ||
    capability.has("api") ||
    provenRisks.some((entry) => entry.modules.includes("api"))
  )
    riskSurfaces.add("api");
  if (
    requestedModules.has("payments") ||
...
function hasProvenFrontendEvidence(profile: ProjectProfile): boolean {
  return (
    (profile.capability_assessments ?? []).some(
      (assessment) => assessment.capability === "frontend" && assessment.status === "PRESENT"
    ) ||
    profile.frameworks.some(
      (record) =>
        PROVEN_CONFIDENCE.has(record.confidence) &&
...
export async function resolveRuntimeCompositionWithRoot(
  root: string,
  modules: ModuleSlug[],
  evidence: CompositionEvidence,
  runtimeRootOverride?: string
  ...
  for (const declaration of manifest.modules) {
    for (const source of declaration.resolvedSources) {
      assertOwnedRuntimePath(
        source.runtimePath,
        UPSTREAM_RUNTIME_SUBTREE,
        `Composition source ${source.provider}/${source.skill}`
      );
  ...
  await Promise.all(
    declaredPaths.map(async (runtimePath) => {
      ...
      if (await regularFileExists(runtimeRoot, absolute)) availableRuntimePaths.add(runtimePath);
    })
  );
  ...
  for (const module of modules) {
    ...
    if (contract !== undefined) {
      assertOwnedRuntimePath(
        ...
        FORGE_CONTRACT_SUBTREE,
        `Forge ${result.workflow} contract for ${module}`
      );
      ...
      for (const eager of result.eager ?? [])
        if (eager.tier === "forge-contract") eager.runtimePath = contractPath;
      const absolute = resolveInside(runtimeRoot, contractPath);
      if (!(await regularFileExists(runtimeRoot, absolute))) result.missing.unshift(contractPath);
    }
    results.push(result);
  }
 ...
 */
export async function resolveRuntimeModuleDependencyClosure(
  root: string,
  modules: readonly ModuleSlug[],
  runtimeRootOverride?: string
): Promise<ModuleDependencyClosure> {
...
async function loadRuntimeCompositionManifest(
  root: string,
  runtimeRootOverride?: string
): Promise<{ manifest: RuntimeCompositionManifest; runtimeRoot: string }> {
...
function parseRuntimeManifest(value: unknown): RuntimeCompositionManifest {
  ...
  for (const module of candidate.modules) {
    ...
    assertOwnedRuntimePath(
      module.forgeContract,
      FORGE_CONTRACT_SUBTREE,
      `Module ${module.module} Forge contract`
    );
    ...
    for (const tier of [module.primary, module.overlays, module.supplemental ?? []]) {
      for (const source of tier) validateSourceWorkflowList(source.workflows, source.skill);
    }
  ...
  const manifest = candidate as RuntimeCompositionManifest;
...
function validateWorkflowMap(value: unknown, label: string): void {
  if (value === undefined) return;
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new Error(`${label} map is invalid`);
  for (const [workflow, path] of Object.entries(value)) {
    if (!(COMPOSITION_WORKFLOWS as readonly string[]).includes(workflow))
...
function assertOwnedRuntimePath(path: string, subtree: string, label: string): void {
  try {
...
function validateSourceWorkflowList(value: unknown, skill: string): void {
  if (value === undefined) return;
  if (
    !Array.isArray(value) ||
    value.length === 0 ||
    ...
    !value.every((workflow) =>
      (COMPOSITION_WORKFLOWS as readonly string[]).includes(workflow as string)
    )
  )
