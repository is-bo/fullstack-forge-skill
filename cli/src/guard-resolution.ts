...
export function createGuardResolver(files: readonly GuardSourceFile[]): GuardResolver {
  const modules: ModuleResolver = createModuleResolver(files, {
    hops: MAX_GUARD_HOPS,
  ...
  const classifyMiddleware = (argument: ts.Expression, file: GuardSourceFile): GuardResolution =>
    classify(argument, file, modules.budgetFor(file), 0);
...
function open(reason: string, budget: ResolutionBudget): GuardResolution {
