...
export async function runShipGates(
  ...
  inventoryOptions: RepositoryInventoryOptions = {}
...
const GATE_MODULES: Record<string, ModuleSlug[]> = {
 ...
 */
export function capabilityApplicability(
  gateId: string,
  profile: ProjectProfile,
  forgeOwned = false
...
function hasApplicationRoute(profile: ProjectProfile): boolean {
  return profile.routes.some((route) => {
    const paths = [route.location, ...route.evidence].filter(
...
async function deriveStableShipState(
  root: string,
  inventoryOptions: RepositoryInventoryOptions = {}
): Promise<StableShipState> {
...
async function revisionWithInventoryOptions(
  root: string,
  inventoryOptions: RepositoryInventoryOptions
): Promise<string> {
