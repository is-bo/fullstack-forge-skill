...
import { bindRuntimeArtifacts, type EvidenceArtifact } from "./evidence-envelope.js";
import {
...
import type {
  ...
  Finding,
  NetworkPolicy,
  PlannedCheck as LedgerPlannedCheck,
  RuntimeEvidence,
  ToolRecord
} from "./types.js";
...
export type BlockedCause =
  "unauthorized" | "offline-policy" | "deselected" | "unavailable" | "failed-closed";
...
export function buildAuditPlan(input: {
  ...
  const policyContext: PolicyContext = input.policy ?? { offline: false, forgeOwned: false };
  const planned: PlannedAuditCheck[] = [];
...
export function isNetworkDependent(command: CommandDefinition): boolean {
...
export function commandNetworkPolicy(
  ...
  if (policy === "UNKNOWN" && isNetworkDependent(command)) return "NETWORK_REQUIRED";
  return policy;
}
...
export async function orchestrateAudit(
  ...
  for (const check of planned) byName.set(check.name, check.id);
  ...
  for (const check of planned) {
    if (skip.has(check.id)) {
      outcomes.push(
        record(input.ledger, check, "explicitly skipped with --skip-check", "deselected")
      );
      continue;
    }
    ...
    if (input.dryRun) {
      outcomes.push(
        record(input.ledger, check, "dry run: planned only, nothing was executed", "deselected")
      );
      continue;
    }
    if (!input.allowRun) {
    ...
    if (input.offline && check.network_policy !== "OFFLINE_SAFE") {
      outcomes.push(
        record(
          input.ledger,
          check,
          check.network_policy === "NETWORK_REQUIRED"
            ? `offline mode refuses '${check.name}' because its definition demonstrably reaches the network`
            : `offline mode refuses '${check.name}' because its network behaviour is UNKNOWN. Fullstack Forge implements no operating-system network isolation, so an arbitrary audited-project command can never be proven offline-safe by inspecting its text.`,
          "offline-policy"
        )
      );
      ...
      if (command === undefined) {
        outcomes.push(
          record(
            input.ledger,
            check,
            "the detected command disappeared before execution",
            "unavailable"
          )
        );
        continue;
    ...
    if (collect === undefined || input.url === undefined) {
      outcomes.push(
        record(
          input.ledger,
          check,
          "no runtime-evidence collector is available in this build",
          "unavailable"
        )
      );
    ...
    try {
      ...
      evidence = {
        ...
        limitations: [
          ...
          `Runtime artifact validation failed: ${(error as Error).message}`
        ],
        complete: false
      };
    }
    ...
    if (evidence.complete) {
      ...
      outcomes.push(
        record(
          input.ledger,
          check,
          `rendered evidence is ${evidence.status}: ${evidence.limitations[0] ?? "no usable capture was produced"}`,
          "failed-closed"
        )
      );
    }
...
function resolveSelectors(
  ...
  for (const value of values ?? []) {
    ...
    throw new Error(
      `Unknown ${flag} value '${value}'. Available checks: ${[...known].join(", ") || "none"}.`
    );
  }
  return resolved;
}

const defaultCommandRunner: CommandRunner = async (command, root) =>
  runFile(command.executable, command.args, root, 15 * 60_000);
 ...
 */
export class ReportAuditLedger implements AuditLedgerSink {
  readonly planned: PlannedAuditCheck[] = [];
  readonly execution: ExecutionRecord[] = [];
  readonly runtime: RuntimeEvidenceRecord[] = [];
  private readonly notRun: Array<{
    check: PlannedAuditCheck;
    reason: string;
    cause: BlockedCause;
  }> = [];
  private plannedChecks: LedgerPlannedCheck[] = [];
  private runtimeLedger: RuntimeEvidence[] = [];
  private toolLedger: ToolRecord[] = [];

  constructor(private readonly revision = "unknown") {}
  ...
  blocked(check: PlannedAuditCheck, reason: string, cause: BlockedCause): void {
    ...
    this.plannedChecks =
      cause === "offline-policy" || cause === "failed-closed"
        ...
        : this.plannedChecks.map((entry) =>
            entry.check_id === check.id ? { ...entry, status: "NOT_RUN", reason } : entry
          );
  }
  ...
  runtimeEvidence(evidence: RuntimeEvidenceRecord): void {
    ...
    this.runtimeLedger = appendRuntimeEvidence(this.runtimeLedger, {
      ...
      artifact_paths: evidence.artifacts.map((artifact) =>
        typeof artifact === "string" ? artifact : artifact.path
      ),
      hashes: evidence.artifacts.flatMap((artifact) =>
        typeof artifact === "string" ? [] : [artifact.sha256]
      ),
      artifacts: evidence.artifacts.flatMap((artifact) =>
  ...
  findings(): Finding[] {
    ...
    for (const [index, entry] of refused.entries()) {
      findings.push({
        ...
        impact:
          "Criteria that depend on this check are unproven and must not be represented as passing.",
        recommendation:
          entry.cause === "unauthorized"
  ...
  residualRisk(): string[] {
    ...
    for (const evidence of this.runtime) {
      lines.push(
        `Runtime evidence (${evidence.kind}) status ${evidence.status}: ${
          evidence.complete
            ? "captured completely"
