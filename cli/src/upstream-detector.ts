...
import type { Finding, Severity, Status } from "./types.js";
...
export type DetectorRun = {
  ...
  unresolved?: { antipattern: string; reason: string }[];
...
const MODULE_ROUTES: { match: RegExp; module: string; section: string }[] = [
  {
 ...
 * their absence.
 */
export function adaptDetectorRun(run: DetectorRun, provenance: DetectorProvenance): AdaptedResult {
  const result: AdaptedResult = { findings: [], advisories: [], notVerified: [], superseded: [] };
    ...
    if (approval !== undefined) {
      result.superseded.push(
        build(raw, provenance, {
          ...
          recommendation:
            "No action: this deviation was explicitly approved and is recorded as such."
        })
      );
      continue;
    }
  ...
  for (const entry of run.unresolved ?? []) {
    result.notVerified.push(
      build(
        { antipattern: entry.antipattern, name: entry.antipattern, description: entry.reason },
        provenance,
        {
          status: "NOT_VERIFIED",
          ...
          recommendation:
            "Verify by rendering the interface, or record the limitation. Do not report this as passing."
        }
      )
    );
  }
...
function isObjective(raw: DetectorFinding): boolean {
  if (raw.advisory === true) return false;
  if (raw.severity === "advisory") return false;
  return OBJECTIVE_CATEGORIES.has(raw.category ?? "");
}

function routeFor(ruleId: string): { module: string; section: string } | undefined {
...
function build(
  ...
  return {
    ...
    location:
      typeof raw.file === "string" && raw.file.length > 0
        ? [
            {
              path: raw.file,
 ...
 */
export function blocksShip(finding: Finding): boolean {
  if (finding.analyzer_id?.startsWith("upstream-detector:") !== true) return true;
