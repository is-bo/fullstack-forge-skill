...
export function missingForDone(
  ...
  const activeAcceptances = feature.risk_acceptances.filter(
    (item) =>
  ...
  const accepted = [...new Set(feature.gate_plan.gates.flatMap((gate) => gate.criteria))].filter(
    (criterion) => {
      const gates =
        feature.gate_plan?.gates.filter((gate) => gate.criteria.includes(criterion)) ?? [];
      if (gates.length === 0 || gates.some((gate) => gate.waiver_policy === "never")) return false;
      const requiredPolicy = gates.some((gate) => gate.waiver_policy === "operational-human")
        ? "operational-human"
        : "advisory";
      return activeAcceptances.some(
        (item) =>
