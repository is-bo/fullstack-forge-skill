 ...
 */
export function inventoryLimitationFinding(
  profile: ProjectProfile,
  section: string
): Finding | undefined {
  const diagnostics = profile.inventory;
  if (diagnostics === undefined || diagnostics.status === "COMPLETE") return undefined;
  ...
  return {
    ...
    impact:
      "Required repository evidence may be missing, so affected criteria and release gates cannot be represented as passed.",
