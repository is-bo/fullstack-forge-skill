 ...
 */
export async function captureRenderedUi(
    ...
    if (offline) {
      if (typeof page.route !== "function") {
        ...
        return {
          ...
          blocked_reason:
            "The resolved browser driver cannot intercept requests, so the offline boundary " +
      ...
      if (typeof page.addInitScript === "function") {
        ...
        limitations.push(
          "WebSocket connections could not be guarded: this driver does not support init " +
            "scripts. Non-loopback WebSocket traffic is NOT_VERIFIED."
        );
      }
      limitations.push(
        "Offline enforcement covers HTTP and HTTPS requests via browser interception and " +
  ...
  for (const request of blockedRequests) {
    limitations.push(
      `Offline policy blocked ${request.resource_type ?? "a"} request to ${request.url}: ${request.reason}.`
    );
  }
...
export async function inspectRenderedUi(
  ...
  if (complete && errors.length > 0) {
    ...
    findings.push({
      ...
      impact:
        "Rendered-state criteria for this route are not supported by complete evidence and must " +
        "not be treated as verified.",
      recommendation:
        "Re-run inspection with the application reachable and every required resource available, " +
    ...
    limitations.push(
      `Capture status ${capture.capture_status}: rendered-state criteria remain NOT_VERIFIED.`
    );
  }
 ...
 */
function exitCodeFor(capture: CaptureOutcome, consoleErrors: number): number {
  if (capture.capture_status === "COMPLETE") return consoleErrors > 0 ? 1 : 0;
  if (capture.capture_status === "BLOCKED") return 2;
  if (capture.capture_status === "FAILED") return 1;
