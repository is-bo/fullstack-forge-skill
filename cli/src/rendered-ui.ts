...
export async function inspectRenderedUi(
  ...
  if (options.dryRun) {
    return {
      value: {
        ...
        dry_run: true,
        artifacts: VIEWPORTS.map((viewport) =>
          toPosix(join(".forge", "evidence", "ui", `${viewport.name}.png`))
        ),
        console_errors: 0,
        console_warnings: 0,
        findings: []
  ...
  await writeFile(
    consolePath,
    `${JSON.stringify({ url, captured_at: utcNow(), entries: consoleEntries }, null, 2)}\n`,
    "utf8"
  );
...
function blocked(reason: string): { value: RenderedUiResult; exitCode: number } {
  return {
    value: {
      tool: "inspect-rendered-ui",
      status: "BLOCKED",
...
async function resolveChromium(
  root: string
): Promise<{ module: MinimalChromium; driver: string } | undefined> {
  ...
  for (const candidate of ["playwright", "@playwright/test", "playwright-core"]) {
    ...
    if (chromium !== undefined) return { module: chromium, driver: candidate };
  }
  return undefined;
}
