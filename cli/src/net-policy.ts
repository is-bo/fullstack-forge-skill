...
export type RequestDecision =
  { allowed: true } | { allowed: false; reason: string; hostClass: HostClass };
...
export function classifyHost(hostname: string): HostClass {
  ...
  if (ipv6 !== undefined) {
    if (ipv6.every((group, index) => (index === 7 ? group === 1 : group === 0))) return "loopback";
    const first = ipv6[0] as number;
 ...
 */
export function decideRequest(rawUrl: string, offline: boolean): RequestDecision {
  if (!offline) return { allowed: true };
  let parsed: URL;
  try {
  ...
  if (hostClass === "loopback") return { allowed: true };
  return {
    allowed: false,
 ...
 */
export function websocketGuardScript(): string {
  return `(() => {
