...
function redactAssignments(text: string): string {
  ...
  output = output.replace(
    new RegExp(
      String.raw`(["']?\b${SENSITIVE_KEY}\b["']?\s*[:=]\s*)(["']?)([^\s"',;&}]+)\2`,
      "giu"
    ),
  ...
  output = output.replace(
    /\b(bearer|basic|digest|token)\s+([A-Za-z0-9._~+/=-]{8,})/giu,
    "$1 [REDACTED]"
  );
  return output;
}
...
function redactTokens(text: string): string {
  ...
  output = output.replace(
    /\beyJ[A-Za-z0-9_-]{4,}\.[A-Za-z0-9_-]{4,}(?:\.[A-Za-z0-9_-]+)?/gu,
    "[REDACTED_JWT]"
  );
  // Common vendor key prefixes carry a secret immediately after the prefix.
  output = output.replace(
    /\b(sk|pk|rk|ghp|gho|ghs|ghu|xox[abprs])[_-][A-Za-z0-9_-]{8,}/gu,
    "[REDACTED_KEY]"
  );
  // Residual high-entropy runs.
  output = output.replace(/\b[A-Za-z0-9+/_-]{32,}={0,2}\b/gu, (match) =>
    looksLikeCredential(match) ? REDACTED : match
  );
  return output;
}
 ...
 */
export function redactText(input: string, maxLength: number = DEFAULT_MAX_LENGTH): Redacted {
 ...
 */
export function redactToString(input: string, maxLength: number = DEFAULT_MAX_LENGTH): string {
  const result = redactText(input, maxLength);
  const notes: string[] = [];
  if (result.redacted) notes.push("redacted");
