...
export function activationWeightFor(evidenceClass: EvidenceClass): number {
  return ACTIVATION_WEIGHTS[evidenceClass];
}

type Classification = { evidence_class: EvidenceClass; reason: string };
 ...
 */
export function classifyEvidencePath(path: string): Classification {
  ...
  if (
    CONFIGURATION_EXTENSIONS.test(name) ||
    name.startsWith(".env") ||
    name === "dockerfile" ||
 ...
 */
export function workspaceForPath(path: string, workspaceRoots: readonly string[]): string {
  const normalized = toPosix(path).replace(/^\.\//u, "");
  let best = ".";
  for (const candidate of workspaceRoots) {
    ...
    if (normalized === root || normalized.startsWith(`${root}/`))
      if (root.length > best.length || best === ".") best = root;
  }
  return best;
}
...
export const CAPABILITY_RULES: readonly CapabilityRule[] = Object.freeze([
  {
    ...
    manifest:
      /["'](?:next-auth|@auth\/core|passport|lucia|@clerk\/[\w-]+|auth0|@auth0\/[\w-]+|bcrypt|bcryptjs|argon2|jsonwebtoken|iron-session|express-session)["']\s*:/iu,
    content:
      /\b(?:signIn|signOut|getServerSession|requireAuth|authenticate|verifyPassword|hashPassword|createSession|refreshToken)\s*\(|\bpassport\.(?:use|authenticate)\b/u
  },
  {
    capability: "authorization",
    content:
      /\b(?:requireRole|hasPermission|checkPermission|authorize|can|enforcePolicy)\s*\(|\brole\s*===?\s*["'`]admin["'`]/u
  },
  {
    capability: "uploads",
    manifest:
      /["'](?:multer|formidable|busboy|@fastify\/multipart|express-fileupload|uppy)["']\s*:/iu,
    content:
      /\bmultipart\/form-data\b|\bmulter\s*\(|\bformidable\s*\(|\bcreatePresignedPost\b|\bgetSignedUrl\s*\(/u
  },
  {
    capability: "payments",
  ...
  },
  {
    capability: "database",
    manifest:
      /["'](?:pg|postgres|mysql2?|sqlite3|better-sqlite3|mongodb|mongoose|@prisma\/client|drizzle-orm|typeorm|sequelize|knex)["']\s*:/iu,
    content:
      /\bmodel\s+\w+\s*\{|\bCREATE\s+TABLE\b|\bALTER\s+TABLE\b|\bnew\s+(?:Pool|Client)\s*\(|\bprisma\.\w+\.(?:findMany|create|update|delete)\b/iu
  },
  {
    capability: "tenancy",
    content:
      /\b(?:tenant_id|tenantId|organization_id|organizationId|workspace_id|workspaceId)\b|\bwhere\s*:\s*\{[^}]*\btenantId\b/u
  },
  {
    capability: "cache",
  ...
  },
  {
    capability: "jobs",
    manifest:
      /["'](?:bullmq|bull|agenda|inngest|@temporalio\/[\w-]+|node-cron|graphile-worker)["']\s*:/iu,
    content: /\bnew\s+(?:Queue|Worker)\s*\(|\bcron\.schedule\s*\(|\bdefineJob\s*\(/u
  },
  {
    capability: "storage",
    manifest:
      /["'](?:@aws-sdk\/client-s3|@google-cloud\/storage|@azure\/storage-blob|cloudinary|minio)["']\s*:/iu,
    content: /\bnew\s+S3Client\s*\(|\bPutObjectCommand\b|\bgetBucket\s*\(/u
  },
  {
    capability: "ai",
    manifest:
      /["'](?:openai|@anthropic-ai\/sdk|@google\/gener\w+-ai|langchain|@langchain\/[\w-]+|ollama)["']\s*:/iu,
    content:
      /\b(?:openai|anthropic)\.(?:chat|messages|completions|responses)\b|\bchat\.completions\.create\s*\(/u
  },
  {
    capability: "realtime",
    manifest:
      /["'](?:socket\.io|socket\.io-client|ws|@supabase\/realtime-js|pusher|ably)["']\s*:/iu,
    content:
      /\bnew\s+WebSocketServer\s*\(|\bio\.on\s*\(\s*["'`]connection|\bnew\s+EventSource\s*\(/u
  },
  {
  ...
  },
  {
    ...
    manifest:
      /["'](?:@opentelemetry\/[\w-]+|@sentry\/[\w-]+|dd-trace|newrelic|prom-client)["']\s*:/iu,
...
function weakerConfidence(confidence: Confidence): Confidence {
...
export type CapabilityEvidence = { capability: string; evidence: DiscoveryEvidence };
...
export function assessCapabilities(
  tagged: readonly CapabilityEvidence[],
...
function decide(
  ...
  if (evidence.length === 0) {
    ...
    status = "PRESENT";
    reasons.push(
      `Activating evidence reached ${score} (threshold ${ACTIVATION_THRESHOLD}) from ${classes.join(", ")}.`
    );
  } else if (score === 0) {
    status = "UNKNOWN";
    reasons.push(
      `Only non-activating evidence was observed (${classes.join(", ")}); this never proves a production capability.`
    );
  } else {
    status = "UNKNOWN";
    reasons.push(
      `Weak evidence totalling ${score} stayed below the activation threshold ${ACTIVATION_THRESHOLD}.`
    );
  }
  for (const neutralized of ["documentation", "test", "fixture", "generated"] as const)
 ...
 */
export async function assessProjectCapabilities(
  rootInput: string,
  workspaceRoots: readonly string[] = [],
