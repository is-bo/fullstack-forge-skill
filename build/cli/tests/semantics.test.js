...
test("a blocked safe fix preserves the original FAIL defect status", async () => {
    await withTemporaryProject("blocked-preserves-fail", async (temporary) => {
        ...
        const fix = await runFile(process.execPath, [cli, "all", "fix", "--safe", "--root", root, "--json"], root);
        ...
        for (const original of failed) {
            ...
            assert.equal(current.status, "FAIL", `${original.id} must remain FAIL after a refused fix; a refused remediation does not unprove the defect`);
        }
    });
});
test("a blocked safe fix records the refusal as a separate fix attempt", async () => {
    await withTemporaryProject("blocked-records-attempt", async (temporary) => {
        ...
        for (const finding of refused) {
            ...
            assert.ok(blocked.every((attempt) => attempt.reason.length > 0), "each blocked attempt states why remediation was refused");
            assert.notEqual(finding.status, "BLOCKED", `${finding.id} defect status must not be overwritten by the fix-attempt status`);
        }
    });
});
...
test("fix without --safe plans only and mutates nothing", async () => {
    await withTemporaryProject("safe-contract-plan", async (temporary) => {
        ...
        const planned = await runFile(process.execPath, [cli, "all", "fix", "--root", root, "--json"], root);
        ...
        assert.deepEqual(await Promise.all(targets.map((path) => readFile(path, "utf8"))), before, "files must be byte-identical after a plan-only run");
    });
});
...
test("two occurrences of one rule in different files become distinct instances", async () => {
    await withTemporaryProject("instance-identity", async (root) => {
        const injection = (name) => `export async function ${name}(req, db) {\n  return db.query(\`SELECT * FROM t WHERE id = \${req.params.id}\`);\n}\n`;
        ...
        for (const finding of sql)
            assert.ok(finding.instance_id?.startsWith("FF-SEC-SQL-001:"), "the stable rule id remains the instance id prefix");
    });
});
test("instance identity is stable when unrelated lines are inserted", async () => {
    await withTemporaryProject("instance-stability", async (root) => {
        ...
        assert.equal(after?.instance_id, before?.instance_id, "moving code within a file must not mint an unrelated identity");
    });
});
test("verifying a resolved instance is not failed by an unrelated instance elsewhere", async () => {
    await withTemporaryProject("instance-verification", async (root) => {
        const vulnerable = (name) => `export async function ${name}(req, db) {\n  return db.query(\`SELECT * FROM t WHERE id = \${req.params.id}\`);\n}\n`;
        ...
        assert.ok(after.some((finding) => finding.status !== "FAIL"), "the resolved instance must be able to leave FAIL even while another instance persists");
        assert.ok(after.some((finding) => finding.status === "FAIL"), "the still-vulnerable instance must remain FAIL");
    });
});
// ---------------------------------------------------------------------------
...
test("verify --dry-run --allow-run executes no project command", async () => {
    await withTemporaryProject("verify-dry-run", async (root) => {
        ...
        await writeFile(join(root, "handler.ts"), "export async function handler(req, db) {\n  return db.query(`SELECT * FROM t WHERE id = ${req.params.id}`);\n}\n", "utf8");
        await runFile(process.execPath, [cli, "security", "audit", "--root", root, "--json"], root);
        await runFile(process.execPath, [cli, "security", "verify", "--dry-run", "--allow-run", "--root", root, "--json"], root);
        const sentinel = await readFile(join(root, "sentinel.txt"), "utf8").catch(() => undefined);
        assert.equal(sentinel, undefined, "a dry run must not execute project commands even when --allow-run is supplied");
    });
});
// ---------------------------------------------------------------------------
...
test("an undeclared nested manifest is not reported as an active workspace", async () => {
    await withTemporaryProject("workspace-undeclared", async (root) => {
        await writeFile(join(root, "package.json"), JSON.stringify({ name: "root-app", version: "1.0.0", workspaces: ["packages/*"] }, null, 2), "utf8");
        await mkdir(join(root, "packages", "declared"), { recursive: true });
        await writeFile(join(root, "packages", "declared", "package.json"), JSON.stringify({ name: "declared-pkg", version: "1.0.0" }, null, 2), "utf8");
        await mkdir(join(root, "examples", "sample"), { recursive: true });
        await writeFile(join(root, "examples", "sample", "package.json"), JSON.stringify({ name: "sample-pkg", version: "1.0.0" }, null, 2), "utf8");
...
test("repository confidence is derived from Git inspection, not an excluded path walk", async () => {
    await withTemporaryProject("repository-confidence", async (root) => {
        await writeFile(join(root, "package.json"), JSON.stringify({ name: "git-app", version: "1.0.0" }, null, 2), "utf8");
        const before = JSON.parse((await runFile(process.execPath, [cli, "discover", "audit", "--root", root, "--json"], root))
            .stdout);
        assert.equal(before.profile.repository.confidence, "MEDIUM", "a non-repository stays MEDIUM");
        await runFile("git", ["init"], root);
        const after = JSON.parse((await runFile(process.execPath, [cli, "discover", "audit", "--root", root, "--json"], root))
            .stdout);
        assert.equal(after.profile.repository.confidence, "HIGH", "an initialised work tree must raise confidence; the old check could never observe this");
