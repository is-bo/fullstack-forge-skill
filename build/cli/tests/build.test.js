...
import { loadFeature, newFeature, saveFeature } from "../src/build-state.js";
...
function evidence(criterion, status, securityControl = false, discipline) {
    return {
        criterion,
...
test("a non-security criterion can be risk-accepted to satisfy done, and is never shown as PASS", async () => {
    await withTemporaryProject("build-accept-ok", async (root) => {
        ...
        assert.equal(accepted.evidence.find((e) => e.criterion === "discipline:observability")?.status, "NOT_VERIFIED");
