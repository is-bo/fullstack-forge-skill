...
import { buildEvidenceClaimDigest, captureEvidenceArtifacts, createBuildEvidenceEnvelope, createEvidenceEnvelope, evidenceClaimDigest, verifyBuildEvidenceEnvelopeIntegrity, verifyEvidenceEnvelope } from "../src/evidence-envelope.js";
...
test("persisted Audit evidence is verified as a diagnostic but never consumed by Ship", async () => {
    await withTemporaryProject("evidence-envelope", async (root) => {
        ...
        const run = (record) => runShipGates(root, profile, createReport(root, profile, [], "audit", [], [], [], undefined, [record], [], revision), [], false);
        ...
        assert.equal((await verifyEvidenceEnvelope({ root, revision, evidence: forged })).verified, false);
        const statusFlip = structuredClone(trusted);
        statusFlip.status = "FAIL";
        assert.equal((await verifyEvidenceEnvelope({ root, revision, evidence: statusFlip })).verified, false);
        const typeSwap = structuredClone(trusted);
        typeSwap.evidence_type = "license-scan";
        assert.equal((await verifyEvidenceEnvelope({ root, revision, evidence: typeSwap })).verified, false);
        const claimMutation = structuredClone(trusted);
        claimMutation.limitations.push("locally appended");
        assert.equal((await verifyEvidenceEnvelope({ root, revision, evidence: claimMutation })).verified, false);
...
test("evidence envelopes reject traversal, missing files, and cross-root reuse", async () => {
    await withTemporaryProject("evidence-envelope-a", async (root) => {
        await writePackage(root);
        const revision = await workingTreeRevision(root);
        await assert.rejects(createEvidenceEnvelope({
            root,
            revision,
            domain: "Audit",
...
test("Build envelopes reject forged claims, roots, revisions, versions, and altered artifacts", async () => {
    await withTemporaryProject("build-envelope-a", async (root) => {
        ...
        assert.equal((await verifyBuildEvidenceEnvelopeIntegrity({ root, revision, claim })).verified, true);
        ...
        assert.equal((await verifyBuildEvidenceEnvelopeIntegrity({ root, revision, claim: handEdited })).verified, false);
        ...
        assert.equal((await verifyBuildEvidenceEnvelopeIntegrity({ root, revision, claim: versionChanged }))
            .verified, false);
        assert.equal((await verifyBuildEvidenceEnvelopeIntegrity({
            root,
        ...
        assert.equal((await verifyBuildEvidenceEnvelopeIntegrity({ root, revision, claim: malformed })).verified, false);
        const unknownField = structuredClone(claim);
        unknownField.envelope.untrusted_extra = true;
        ...
        assert.equal((await verifyBuildEvidenceEnvelopeIntegrity({ root, revision, claim })).verified, false);
    });
});
test("reasoned Build exclusions are producer-restricted and bind the exclusion reason", async () => {
    await withTemporaryProject("build-envelope-na", async (root) => {
        ...
        assert.equal((await verifyBuildEvidenceEnvelopeIntegrity({ root, revision, claim: edited })).verified, false);
        await assert.rejects(createBuildEvidenceEnvelope({
            root,
            revision,
...
test("rendered-UI PASS binds the complete typed state and viewport matrix", async () => {
    await withTemporaryProject("build-envelope-runtime", async (root) => {
        ...
        const revision = await workingTreeRevision(root);
        const recordedAt = new Date().toISOString();
        const runtime = BUILD_RUNTIME_STATES.flatMap((state) => BUILD_RUNTIME_VIEWPORTS.map((viewport) => ({
...
test("Ship command envelopes reject command and output claim replay", async () => {
    await withTemporaryProject("ship-command-envelope", async (root) => {
        ...
        assert.equal((await verifyEvidenceEnvelope({ root, revision, evidence: commandSwap })).verified, false);
        const outputMutation = structuredClone(evidence);
        outputMutation.command.output_sha256 = sha256("forged output");
        assert.equal((await verifyEvidenceEnvelope({ root, revision, evidence: outputMutation })).verified, false);
    });
});
async function auditEvidence(root, revision) {
    const record = auditClaim(revision);
...
async function shipCommandEvidence(root, revision) {
