    ...
    for (const name of fixtureNames) {
        await t.test(name, async () => {
            await withTemporaryProject(`fixture-discovery-${name}`, async (temporary) => {
                ...
                const expected = JSON.parse(await readFile(join(root, "expected-findings.json"), "utf8"));
                ...
                for (const [tool, minimum] of Object.entries(expected.tools)) {
                    const result = await inspectWithTool(tool, root);
                    assert.ok(result.observations.length + result.findings.length >= minimum, `${name} ${tool} expected at least ${minimum} signal(s)`);
                }
            });
        });
    }
...
test("frontend discovery does not imply a public indexable website", async () => {
    await withTemporaryProject("profile-frontend-visibility", async (root) => {
        ...
        await writeFile(join(root, "Dashboard.tsx"), "export const Dashboard = () => <main />;\n", "utf8");
        const profile = await discoverProject(root);
...
test("Django URLconfs reconcile backend framework, route, API assessment, and request risk", async () => {
    await withTemporaryProject("profile-django-route", async (root) => {
        ...
        await writeFile(join(project, "urls.py"), `from django.urls import path
from . import views
...
`, "utf8");
        ...
        assert.ok(profile.routes.some((route) => route.name === "ROUTE /patients/" &&
            route.evidence.some((item) => item === "adapter: django-urlconf")));
        assert.equal(profile.capability_assessments?.find((assessment) => assessment.capability === "api" && assessment.workspace === ".")?.status, "PRESENT");
        assert.ok(profile.risk_evidence?.some((risk) => risk.risk === "request-boundary" && risk.path === "project/urls.py"));
    });
});
test("Next.js DELETE handlers produce modeled API and destructive-route evidence", async () => {
    await withTemporaryProject("profile-next-delete", async (root) => {
        ...
        await writeFile(join(root, "package.json"), `${JSON.stringify({ name: "next-api", dependencies: { next: "0.0.0-fixture" } })}\n`, "utf8");
        await writeFile(join(routeDirectory, "route.ts"), `export async function DELETE() {
...
`, "utf8");
        ...
        for (const risk of ["request-boundary", "destructive-or-administrative-route"])
            assert.ok(profile.risk_evidence?.some((item) => item.risk === risk), risk);
    });
});
test("project profile schema v2 records applications, routes, boundaries, and old-profile regeneration", async () => {
    await withTemporaryProject("profile-v2", async (root) => {
        ...
        await writeFile(join(root, "server.ts"), `app.get("/dashboard", (_req, res) => res.send("ok"));
...
`, "utf8");
        ...
        assert.equal(profile.routes.find((route) => route.name === "GET /dashboard")?.visibility, "unknown");
        assert.equal(profile.routes.find((route) => route.name === "GET /health")?.visibility, "public");
        ...
        await writeFile(join(root, ".forge", "project-profile.json"), `${JSON.stringify({ schema_version: 1, root, generated_at: new Date().toISOString(), detections: [], capabilities: {} })}\n`, "utf8");
        ...
        const regenerated = JSON.parse(await readFile(join(root, ".forge", "project-profile.json"), "utf8"));
        assert.equal(regenerated.schema_version, 2);
        assert.equal(regenerated.repository.name, "profile-app");
        assert.equal(JSON.parse(await readFile(join(root, ".forge", "project-profile.schema-v1.json"), "utf8")).schema_version, 1);
