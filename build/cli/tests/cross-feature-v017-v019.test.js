...
import { ReportAuditLedger, buildAuditPlan, commandNetworkPolicy, orchestrateAudit } from "../src/audit-orchestration.js";
 ...
 */
const arbitrary = (name, definition) => ({
    name,
    executable: "npm",
    args: ["run", name],
...
test("orchestration never invents OFFLINE_SAFE for an audited project's own scripts", () => {
    ...
    for (const definition of ["eslint .", "vitest run", "tsc -p .", "node ./tools/check.js"]) {
        ...
        assert.equal(classifyCommandNetworkPolicy(command, { offline: true, forgeOwned: false }), "UNKNOWN");
...
test("keyword scanning may escalate to NETWORK_REQUIRED but never downgrades", () => {
    assert.equal(commandNetworkPolicy(arbitrary("deps", "npm audit"), { offline: true, forgeOwned: false }), "NETWORK_REQUIRED");
    // Escalation is the only direction available: nothing here can reach OFFLINE_SAFE.
    const policies = ["eslint .", "npm audit", "curl https://example.test", "echo hi"].map((definition) => commandNetworkPolicy(arbitrary("c", definition), { offline: true, forgeOwned: false }));
...
test("a blocked offline check never produces PASS gate evidence and stays out of the fix set", async () => {
    ...
    const executed = [];
    const result = await orchestrateAudit(input({
        allowRun: true,
        offline: true,
        ledger,
        runCommand: (command) => {
    ...
    const report = createReport("/tmp/project", profile("/tmp/project"), ledger.findings(), "security", result.execution, [], ledger.residualRisk(), undefined, [], [], "rev-1", undefined, ledger.ledgers());
    assert.equal(report.findings.some((finding) => finding.status === "PASS"), false);
    assert.equal(report.planned_checks.every((check) => check.status !== "RUN"), true);
});
test("an unauthorized check is NOT_RUN, never BLOCKED, so `forge fix` cannot claim it", async () => {
    ...
    for (const finding of ledger.findings())
        assert.notEqual(finding.status, "BLOCKED");
});
test("a normal audit stays static-only: nothing executes without --allow-run", async () => {
    ...
    assert.deepEqual(ran.map((outcome) => outcome.id), ["module:security"]);
});
