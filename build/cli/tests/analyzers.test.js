...
test("an ordinary exported handler is not a background-job signal", async () => {
    await withTemporaryProject("analyzer-exported-handler", async (root) => {
        await writeFile(join(root, "api.ts"), "export async function listInvoices() { return db.invoice.findMany({ where: {} }); }\n", "utf8");
        assert.ok(!(await findingIds(root, "tenancy")).has("FF-TENANT-BACKGROUND-001"));
    });
});
...
test("boundary analyzers do not treat vulnerable fixtures as production behavior", async () => {
    await withTemporaryProject("analyzer-fixture-boundary", async (root) => {
        ...
        await writeFile(join(root, "fixtures", "upload.ts"), `app.post("/upload", upload.any(), async (req, res) => {
...
test("cache analyzer preserves confirmed unscoped literal failures", async () => {
    const findings = await cacheFindings("cache-literal", `export async function load(userId: string, tenantId: string) {
...
await redis.get((key));`
    ...
    for (const [index, body] of safeSources.entries()) {
        const findings = await cacheFindings(`cache-safe-${index}`, `export async function load(userId: string, tenantId: string) { ${body} }`);
        assert.deepEqual(findings, [], `safe shape ${index}`);
    }
});
test("cache analyzer keeps tenant and user scope findings independent", async () => {
    const missingUser = await cacheFindings("cache-missing-user", `export async function load(userId: string, tenantId: string) {
  ...
  await redis.get(key);
    ...
    const missingTenant = await cacheFindings("cache-missing-tenant", `export async function load(userId: string, tenantId: string) {
...
test("cache analyzer follows straight-line mutable reassignment without trusting the initializer", async () => {
    const findings = await cacheFindings("cache-reassigned", `export async function load(userId: string, tenantId: string) {
...
if (flag) key = "dashboard";`
        ...
        const findings = await cacheFindings(`cache-unresolved-${name}`, `export async function load(userId: string, tenantId: string) {
  ${body}
  await redis.get(key);
}`);
...
test("cache analyzer stays bounded across cycles, deep aliases, large files, and shadowed names", async () => {
    const deepAliases = Array.from({ length: 24 }, (_, index) => `const key${index + 1} = key${index};`).join("\n");
    const filler = Array.from({ length: 2500 }, (_, index) => `const filler${index} = ${index};`).join("\n");
    const findings = await cacheFindings("cache-bounded", `export async function load(userId: string, tenantId: string) {
  ${filler}
  let cycleA: string;
  let cycleB: string;
  cycleA = cycleB;
...
test("SQL sink analysis distinguishes SQL structure from bound values", async () => {
    const cases = [
        {
            ...
            source: "export const run = (req) => pool.query(`SELECT * FROM users WHERE name = '${req.query.term}'`);",
            expected: "FAIL"
        },
        {
            name: "safe-positional",
            source: 'export const run = (req) => pool.query("SELECT * FROM users WHERE name = $1", [req.query.term]);',
            expected: "CLEAN"
        },
        {
            name: "safe-ilike-template-value",
            source: 'export const run = (req) => pool.query("SELECT * FROM users WHERE name ILIKE $1", [`%${req.query.term}%`]);',
            expected: "CLEAN"
        },
        {
            name: "safe-client",
            source: 'export const run = (req) => client.query("SELECT * FROM users WHERE name = $1", [req.query.term]);',
            expected: "CLEAN"
        },
        {
            name: "safe-execute",
            source: 'export const run = (req) => db.execute("SELECT * FROM users WHERE name = ?", [req.query.term]);',
            expected: "CLEAN"
        },
        {
            name: "safe-prisma-tag",
            source: "export const run = (req) => prisma.$queryRaw`SELECT * FROM users WHERE name = ${req.query.term}`;",
            expected: "CLEAN"
        },
        {
            name: "unsafe-prisma-raw",
            source: "export const run = (req) => prisma.$queryRawUnsafe(`SELECT * FROM users WHERE name = '${req.query.term}'`);",
            expected: "FAIL"
        },
        {
            name: "unknown-wrapper",
            source: "export const run = (req) => customDb.search('SELECT * FROM users WHERE name = ?', [req.query.term]);",
    ...
    for (const fixture of cases) {
        ...
        if (fixture.expected === "CLEAN") {
            ...
            assert.ok(sql.some((finding) => finding.id === "FF-SEC-SQL-001" && finding.status === "FAIL"), fixture.name);
        }
        else {
            assert.ok(sql.some((finding) => finding.id === "FF-SEC-SQL-NOT-VERIFIED-001" && finding.status === "NOT_VERIFIED"), fixture.name);
        }
    }
});
test("tenant scope maps raw SQL placeholders to authenticated tenant context", async () => {
    const safe = await analyzerFindings("tenant-safe-raw", `export async function load(req) {
  return pool.query("SELECT id FROM patients WHERE clinicId = $1", [req.session.user.clinicId]);
    ...
    const unsafe = await analyzerFindings("tenant-unsafe-raw", `export async function load(req, clinicId) {
  return pool.query("SELECT id FROM patients WHERE id = $1", [req.params.id]);
    ...
    const unresolved = await analyzerFindings("tenant-unresolved-raw", `export async function load(req) {
  const clinicId = resolveClinic(req);
  return pool.query("SELECT id FROM patients WHERE clinicId = $1", [clinicId]);
}`, "tenancy");
    assert.ok(unresolved.some((finding) => finding.id === "FF-TENANT-SCOPE-NOT-VERIFIED-001" && finding.status === "NOT_VERIFIED"));
...
test("common domain tenant keys preserve raw SQL scope outcomes", async () => {
    for (const key of [
        ...
        const findings = await analyzerFindings(`tenant-key-${key}`, `export async function load(req) {
...
test("sensitive Express routes require resolvable authorization", async () => {
    const unprotected = await analyzerFindings("authz-route-unprotected", `router.delete("/admin/patients/:id", async (req, res) => {
  await prisma.patient.delete({ where: { id: req.params.id } });
  res.sendStatus(204);
});`, "authorization");
    assert.ok(unprotected.some((finding) => finding.id === "FF-AUTHZ-ROUTE-001"));
    const protectedRoute = await analyzerFindings("authz-route-protected", `router.delete("/admin/patients/:id", requireRole("admin"), async (req, res) => {
  await prisma.patient.delete({ where: { id: req.params.id } });
  res.sendStatus(204);
});`, "authorization");
    assert.ok(!protectedRoute.some((finding) => finding.id === "FF-AUTHZ-ROUTE-001"));
    const publicRoute = await analyzerFindings("authz-route-public", 'router.get("/health", (_req, res) => res.sendStatus(200));', "authorization");
    assert.deepEqual(publicRoute, []);
    const unresolved = await analyzerFindings("authz-route-unresolved", `router.use(globalGuard);
router.delete("/admin/patients/:id", deletePatient);`, "authorization");
    assert.ok(unresolved.some((finding) => finding.id === "FF-AUTHZ-NOT-VERIFIED-001" && finding.status === "NOT_VERIFIED"));
});
test("accessibility analyzer covers image alternatives and keyboard semantics", async () => {
    const unsafe = await analyzerFindings("a11y-structural-unsafe", `export const Card = () => <><img src="doctor.png" /><div onClick={open}>Open</div></>;`, "accessibility", "view.tsx");
    ...
    for (const [name, source] of [
        ...
        assert.deepEqual(await analyzerFindings(`a11y-${name}`, source, "accessibility", "view.tsx"), [], name);
    }
});
test("every upload rule fires and a supported hardened flow stays clean", async () => {
    ...
    for (const [name, id, source] of cases) {
        ...
        assert.ok(findings.some((finding) => finding.id === id), name);
    }
    const hardened = await analyzerFindings("upload-hardened", `const upload = multer({ limits: { fileSize: maxBytes, files: 1 } });
...
async function analyzerFindings(name, source, section, file = "route.ts") {
    let findings = [];
...
async function cacheFindings(name, source) {
    let findings = [];
    await withTemporaryProject(name, async (root) => {
...
async function authorizationIds(name, source) {
    let ids = new Set();
    await withTemporaryProject(name, async (root) => {
...
test("authorization ignores unrelated policy strings and unused imports", async () => {
    for (const [name, prelude] of [
        ...
        const ids = await authorizationIds(`authz-${name}`, `${prelude}
...
test("authorization after release or for another object does not protect the lookup", async () => {
    const after = await authorizationIds("authz-after-release", `export async function load(req, res) {
  ...
  await authorize(req.user, req.params.id);
    ...
    const different = await authorizationIds("authz-different-object", `export async function load(req) {
...
test("authorization accepts a connected owner predicate and dominating policy guard", async () => {
    const owner = await authorizationIds("authz-owner-predicate", `export function load(req) {
  return prisma.record.findUnique({
    where: { id: req.params.id, ownerId: req.user.id }
  });
}`);
    assert.ok(!owner.has("FF-AUTHZ-OBJECT-001"));
    const guard = await authorizationIds("authz-dominating-guard", `export async function load(req) {
...
test("authorization requires both subject and object connection", async () => {
    const missingSubject = await authorizationIds("authz-no-subject", `export async function load(req) {
  if (!(await canAccess(req.params.id))) throw new Error("forbidden");
  return prisma.record.findUnique({ where: { id: req.params.id } });
}`);
    assert.ok(missingSubject.has("FF-AUTHZ-OBJECT-001"));
    const crossTenant = await authorizationIds("authz-cross-tenant", `export function load(req) {
  return prisma.record.findUnique({
...
test("conditional or non-dominating authorization calls do not protect a lookup", async () => {
    const conditionalCall = await authorizationIds("authz-conditional-call", `export async function load(req) {
  if (req.query.check) await authorize(req.user, req.params.id);
  return prisma.record.findUnique({ where: { id: req.params.id } });
}`);
    assert.ok(conditionalCall.has("FF-AUTHZ-OBJECT-001"));
    const conditionalExit = await authorizationIds("authz-conditional-exit", `export async function load(req) {
...
test("subject-shaped output fields are not query authorization predicates", async () => {
    const ids = await authorizationIds("authz-output-owner", `export function load(req) {
  return prisma.record.findUnique({
