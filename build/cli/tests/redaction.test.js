...
import { DEFAULT_MAX_LENGTH, redactError, redactText, redactToString, redactUrl } from "../src/redaction.js";
/**
...
const JWT = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJTRU5USU5FTCJ9.sentinel-signature-000011112222";
function assertClean(haystack, sentinel, context) {
...
test("URL query values, fragments, and userinfo are redacted", () => {
    const redacted = redactUrl(`http://admin:${SENTINELS.password}@127.0.0.1:3000/report?token=${SENTINELS.query}#${SENTINELS.fragment}`);
...
test("URLs embedded in console text are redacted in place", () => {
    const result = redactText(`Failed to load http://127.0.0.1:3000/api?apiKey=${SENTINELS.apiKey} (net::ERR_FAILED)`);
...
test("websocket URLs are redacted like any other URL", () => {
    const result = redactText(`WebSocket connection to wss://example.com/live?access_token=${SENTINELS.websocket} failed`);
