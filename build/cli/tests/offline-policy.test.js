...
import { classifyCommandNetworkPolicy, decideCommandExecution, isUnreachableRegistry } from "../src/offline-policy.js";
...
function projectCommand(name, definition) {
    return {
        name,
        executable: "npm",
        args: ["run", name],
...
test("a protective-sounding script name never earns an offline exemption", () => {
    for (const name of ["offline", "verify:offline", "test:offline", "safe-check"]) {
        ...
        assert.equal(classifyCommandNetworkPolicy(command, OFFLINE), "UNKNOWN", `${name} must be classified from its definition, not its name`);
    }
});
test("an explicitly designed cache-only installation check is permitted offline", () => {
    const command = projectCommand("offline:install", "npm install --offline --ignore-scripts --registry=http://127.0.0.1:9/ ./package.tgz");
...
test("a cache-only claim requires both the offline flag and an unreachable registry", () => {
    ...
    const withoutOffline = projectCommand("b", "npm install --registry=http://127.0.0.1:9/ ./package.tgz");
    const reachableRegistry = projectCommand("c", "npm install --offline --registry=https://registry.npmjs.org/ ./package.tgz");
...
test("Forge-owned repository scripts are offline-safe only inside the Forge package root", () => {
    ...
    assert.equal(classifyCommandNetworkPolicy(command, { offline: true, forgeOwned: true }), "forge-internal-offline-safe");
    assert.equal(classifyCommandNetworkPolicy(command, OFFLINE), "UNKNOWN", "an audited project reusing the script name inherits no exemption");
    assert.equal(decideCommandExecution(command, { offline: true, forgeOwned: true }).permitted, true);
});
...
test("run-project-command --offline --allow-run blocks an unknown script and never executes it", async () => {
    await withTemporaryProject("offline-run-project-command", async (root) => {
        ...
        assert.equal(record.sandbox, "none", "no operating-system network isolation exists, so none may be claimed");
...
test("run-project-command records why an online execution ran", async () => {
    await withTemporaryProject("online-run-project-command", async (root) => {
        await writeFile(join(root, "package.json"), `${JSON.stringify({ name: "ordinary-project", private: true, scripts: { noop: 'node -e "0"' } }, null, 2)}\n`, "utf8");
...
test("ship --offline --allow-run blocks unknown project commands and cannot pass", async () => {
    await withTemporaryProject("offline-ship", async (root) => {
        ...
        await assert.rejects(readFile(join(root, "should-not-run.txt"), "utf8"), "an offline-blocked command must never be spawned");
...
function options(root, offline, allowRun) {
    return {
        cwd: root,
        json: true,
        dryRun: false,
