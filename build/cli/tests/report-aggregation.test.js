...
function finding(id, severity, status, overrides = {}) {
    return {
        id,
        section: "database",
        title: `Finding ${id}`,
...
const unverifiedCritical = finding("FF-DATA-TRANSACTION-NOT-VERIFIED-001", "CRITICAL", "NOT_VERIFIED");
...
test("a CRITICAL, LOW-confidence NOT_VERIFIED finding is never counted as a confirmed defect", () => {
    ...
    assert.equal(summary.confirmed_critical, 0, "an unverified finding must not reach the critical count");
    ...
    assert.equal(summary.unverified_critical_or_high, 1, "the gap must still be visible, just not as a defect");
...
test("every status is aggregated into exactly one verdict class", () => {
    ...
    const classified = Object.values(summary.by_class).reduce((total, bucket) => total + bucket.total, 0);
    ...
    assert.equal(summary.by_class.evidence_gap.total, 2, "NOT_VERIFIED and BLOCKED are evidence gaps");
...
test("the summary publishes no severity total that spans verdict classes", () => {
    const summary = summarizeFindings([unverifiedCritical, confirmedHigh]);
    assert.ok(!Object.prototype.hasOwnProperty.call(summary, "by_severity"), "a class-spanning severity rollup is exactly the number that must not exist");
});
test("aggregation preserves severity, confidence, and status as potential impact and verdict", () => {
    ...
    assert.ok(stored);
    assert.equal(stored.severity, "CRITICAL", "severity is potential impact and must not be downgraded");
    assert.equal(stored.confidence, "LOW");
    assert.equal(stored.status, "NOT_VERIFIED", "the status contract must not be weakened to improve a score");
});
...
test("confirmed defects rank above unproven findings of higher potential impact", () => {
    const report = createReport("/project", profile, [unverifiedCritical, confirmedHigh], "database");
    assert.equal(report.findings[0]?.id, confirmedHigh.id, "an unverified CRITICAL must not head the list a reader skims as confirmed defects");
    assert.equal(report.findings[1]?.id, unverifiedCritical.id, "nothing may be dropped from the list");
});
...
test("markdown remediation and not-run sections stay on opposite sides of the verdict line", () => {
    const report = createReport("/project", profile, [unverifiedCritical, confirmedHigh], "database");
    const markdown = renderMarkdown(report);
    const remediation = markdown.slice(markdown.indexOf("## Prioritized remediation plan"), markdown.indexOf("## Execution ledger"));
    assert.ok(remediation.includes(confirmedHigh.id), "a confirmed defect belongs in remediation");
    assert.ok(!remediation.includes(unverifiedCritical.id), "an unverified finding must not appear as remediable confirmed work");
    const notRun = markdown.slice(markdown.indexOf("## Checks not run or not verified"));
    assert.ok(notRun.includes(unverifiedCritical.id), "the gap must be named where gaps are reported");
});
