...
import { REPORT_SCHEMA_VERSION, createReport, migrateReport, readReport, renderMarkdown, writeReport } from "../src/report.js";
...
function legacyReport(root, release) {
...
for (const release of ["v0.1.3", "v0.1.4", "v0.1.5", "v0.1.6"]) {
    test(`a ${release} report migrates without losing findings or fabricating ledgers`, async () => {
        await withTemporaryProject(`migrate-${release}`, async (root) => {
            ...
            assert.ok(migration.notes.some((note) => note.includes("not evidence that")), "the report must say that an empty ledger does not imply a passing check");
...
test("JSON and Markdown stay synchronized across every new ledger", async () => {
    await withTemporaryProject("report-sync", async (root) => {
        ...
        const report = createReport(root, localProfile, [finding], "security", [], [], [], undefined, [], [], "tree:abc", undefined, {
            module_decisions: [
                {
                    module: "ui",
        ...
        assert.match(markdown, /NOT audited in this run — this is not evidence that the module is inapplicable/u);
...
test("a report from a newer schema version is refused rather than misread", () => {
    assert.throws(() => migrateReport({ ...legacyReport("/project", "v0.1.6"), schema_version: 99 }), /newer than the supported version/u);
    assert.throws(() => migrateReport({ nonsense: true }), /Unsupported or invalid/u);
});
//# sourceMappingURL=report-schema.test.js.map