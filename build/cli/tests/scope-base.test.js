...
async function initRepository(root) {
    ...
    await writeFile(join(root, "package.json"), `${JSON.stringify({ name: "scope-fixture", private: true }, null, 2)}\n`, "utf8");
...
test("a committed change on a branch is visible against the local default base", async () => {
    await withTemporaryProject("scope-committed", async (root) => {
        ...
        assert.ok(scope.evidence.changed_files.some((file) => file.path === "index.ts"), "the committed branch change must appear in the changed set");
    });
});
test("changed scope blocks with a structured error when no base can be resolved", async () => {
    await withTemporaryProject("scope-no-base", async (root) => {
        ...
        await git(root, ["commit", "-m", "initial"]);
        const profile = await discoverProject(root);
        await assert.rejects(() => analyzeChangedScope(root, profile), (error) => {
