...
test("a detected-host selector installs only the requested finite host set", async () => {
    ...
    await withTemporaryProject("detected-host-footprint", async (root) => {
        ...
        assert.ok(selected.every((action) => action.path === "CLAUDE.md" ||
            ...
            action.path.startsWith(".fullstack-forge/")), `unexpected install path: ${selected.map((action) => action.path).join(", ")}`);
        for (const unselected of [".agents/", ".gemini/", ".github/", ".windsurf/"])
            assert.ok(selected.every((action) => !action.path.startsWith(unselected)), `${unselected} must not be installed by a claude,cursor selector`);
