...
async function project(root, files) {
...
async function inspect(root, section) {
...
test("an unprotected destructive route is analyzed when no authorization control exists", async () => {
    await withTemporaryProject("readiness-v1-absent", async (root) => {
        ...
        assert.ok(findings.some((finding) => finding.id === "FF-AUTHZ-ROUTE-001" && finding.status === "FAIL"), "the missing route guard must be reported");
        assert.ok(!findings.some((finding) => finding.status === "NOT_APPLICABLE"), "an observed risk surface must never yield a non-applicability verdict");
    });
});
test("an unused authorization helper does not change authorization findings", async () => {
    const run = async (extra) => withTemporaryProject("readiness-v1-dead", async (root) => {
    ...
    const withDeadHelper = await run({
        "src/unrelated.ts": "export function requireRole(user: { role: string }, role: string) { return user.role === role; }\n"
    });
...
test("module decisions, execution, and finding status cannot disagree", async () => {
    await withTemporaryProject("readiness-v1-agree", async (root) => {
        ...
        const result = await inspectSection("authorization", root, profile, undefined, undefined, decision);
        assert.ok(!result.findings.some((finding) => finding.status === "NOT_APPLICABLE"), "an APPLICABLE decision must not produce a non-applicability finding");
    });
});
...
`
            ...
            assert.ok(findings.some((finding) => finding.id === "FF-TENANT-SCOPE-001" && finding.status === "FAIL"), `the unscoped read must be detected for '${key}'`);
        });
    });
...
`;
async function authorizationFindings(root) {
    await project(root, {
        "src/db.ts": DB,
        "src/guards.ts": GUARDS,
        "src/dynamic.ts": 'const registry: Record<string, unknown> = {};\nexport function dynamicGuard(): any { return (registry as any)["guard"]; }\n',
...
test("structurally proven route guards are not reported, whatever they are named", async () => {
    await withTemporaryProject("readiness-v3-guards", async (root) => {
        ...
        const routeLines = new Set(findings
        ...
        assert.equal(routeLines.size, 1, `unexpected route findings at lines ${[...routeLines].join(", ")}`);
        assert.ok(findings.some((finding) => finding.id === "FF-AUTHZ-ROUTE-001" && finding.status === "FAIL"));
    });
});
...
`
        ...
        assert.ok(!findings.some((finding) => finding.id === "FF-AUTHZ-OBJECT-001"), "router.delete must not raise object authorization");
    });
});
test("inline and aliased ownership predicates both count as authorization", async () => {
    await withTemporaryProject("readiness-v3-ownership", async (root) => {
        const findings = await authorizationFindings(root);
        const objectLines = new Set(findings
...
function applicabilityFinding(section) {
    return {
...
function verdict(section, status) {
    return {
...
for (const status of ["FAIL", "WARNING", "NOT_VERIFIED", "PASS", "BLOCKED"])
    test(`a synthetic non-applicability verdict is superseded by an active ${status}`, async () => {
        await withTemporaryProject(`readiness-v4-${status}`, async (root) => {
            ...
            const { profile } = await discoverProjectWithInventory(root);
            const report = createReport(root, profile, [applicabilityFinding("authorization"), verdict("authorization", status)], "test");
            ...
            assert.equal(active.filter((finding) => finding.status === "NOT_APPLICABLE").length, 0, "no active non-applicability verdict may remain");
...
test("distinct findings in one section are never suppressed by the invariant", async () => {
    await withTemporaryProject("readiness-v4-distinct", async (root) => {
        await project(root, { "src/app.ts": "export const a = 1;\n" });
        const { profile } = await discoverProjectWithInventory(root);
        const report = createReport(root, profile, [
        ...
        const active = report.findings.filter((finding) => finding.section === "security" && finding.status !== "SUPERSEDED");
...
`
        ...
        for (const finding of findings.filter((candidate) => candidate.id === "FF-SEC-SQL-NOT-VERIFIED-001"))
            assert.equal(finding.status, "NOT_VERIFIED");
