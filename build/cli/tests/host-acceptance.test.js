...
import { CANONICAL_ROOT_POSIX, extractFrontmatter, isAdapter, readAdapterMarker } from "../src/managed-layout.js";
 ...
 */
async function resolveThroughHost(root, host, skill) {
    ...
    try {
        ...
        throw new Error(`${host.id} discovery failed: no adapter at ${adapterPath} (${error.message})`, { cause: error });
    }
    ...
    try {
        ...
        throw new Error(`${host.id} could not follow the pointer for ${skill}: ${pointer} from ${dirname(adapterPath)} does not resolve to readable canonical content (${error.message})`, { cause: error });
    }
    if (isAdapter(canonicalText))
        throw new Error(`${host.id} pointer for ${skill} resolves to another adapter, not the playbook`);
...
async function discoverSkills(root, host) {
    const discoveryRoot = join(root, ...host.skillsRoot);
    const names = [];
...
async function canonicalSource(relative) {
 ...
 */
function referencedResources(playbook) {
    const pattern = /fullstack-forge\/(?:references|schemas|templates|profiles|checklists)\/[^\s`)"']+/gu;
...
async function withSpacedProject(callback) {
    ...
    if (!basename(parent).startsWith("fullstack-forge-host-space-") ||
    ...
    await withTemporaryProject("host-discovery", async (root) => {
        ...
        for (const host of HOSTS) {
            await t.test(`${host.id} (${host.product}) — live host loader NOT_VERIFIED`, async () => {
                ...
                assert.equal(skills.length, EXPECTED_SKILLS, `${host.id} discovery root holds ${skills.length} skills`);
                ...
                for (const skill of skills) {
                    ...
                    assert.equal(name, skill, `${host.id}/${skill} frontmatter name must match its directory`);
                    assert.match(resolved.frontmatter, /^description:\s*\S/mu, `${host.id}/${skill} needs a description to trigger on`);
                }
        ...
        for (const host of HOSTS) {
            await t.test(host.id, async () => {
                for (const skill of await discoverSkills(root, host)) {
                    const resolved = await resolveThroughHost(root, host, skill);
                    const canonical = extractFrontmatter((await canonicalSource(`${skill}/SKILL.md`)).toString("utf8"), `canonical:${skill}`);
                    assert.equal(sha256(resolved.frontmatter), sha256(canonical), `${host.id}/${skill} adapter frontmatter drifted from canonical`);
                }
            });
        }
    });
    ...
    await withTemporaryProject("host-pointer", async (root) => {
        ...
        for (const host of HOSTS) {
            await t.test(host.id, async () => {
                for (const skill of await discoverSkills(root, host)) {
                    const resolved = await resolveThroughHost(root, host, skill);
                    assert.equal(resolved.canonicalPath, join(canonicalRoot, skill, "SKILL.md"), `${host.id}/${skill} pointer must land on the shared canonical copy`);
                    ...
                    assert.equal(sha256(Buffer.from(resolved.canonicalText, "utf8")), sha256(await canonicalSource(`${skill}/SKILL.md`)));
...
test("host acceptance 4 (simulated): every resource a canonical playbook names resolves from the canonical root", async () => {
    await withTemporaryProject("host-references", async (root) => {
        ...
        const host = HOSTS.find((entry) => entry.id === "claude");
        let checked = 0;
        for (const skill of await discoverSkills(root, host)) {
            const resolved = await resolveThroughHost(root, host, skill);
            ...
            for (const reference of referencedResources(resolved.canonicalText)) {
                ...
                assert.ok(target.startsWith(canonicalRoot + sep), `${skill} names a reference outside the canonical root: ${reference}`);
                await stat(target).catch((error) => {
                    throw new Error(`${skill} names ${reference}, which does not resolve under ${canonicalRoot} (${error.message})`);
                });
                checked += 1;
            }
        }
...
test("host acceptance 5 (simulated): missing canonical content is a clear damaged installation, never a silent pass", async () => {
    await withTemporaryProject("host-damaged", async (root) => {
        ...
        await assert.rejects(resolveThroughHost(root, host, "forge-api"), /could not follow the pointer for forge-api/u);
        ...
        const doctor = await runFile(process.execPath, [CLI, "doctor", "--offline", "--root", root, "--json"], root, 180_000);
...
test("host acceptance 6 (simulated): updating one host never removes canonical content another host still needs", async () => {
    await withTemporaryProject("host-update-isolation", async (root) => {
        ...
        const claude = HOSTS.find((entry) => entry.id === "claude");
        ...
        assert.ok(actions.every((action) => action.action !== "remove"), "updating one host must not retire content");
...
test("host acceptance 7 (simulated): uninstalling one host retains the canonical content the remaining hosts need", async () => {
    await withTemporaryProject("host-uninstall-shared", async (root) => {
        ...
        const cursor = HOSTS.find((entry) => entry.id === "cursor");
        const gemini = HOSTS.find((entry) => entry.id === "gemini");
...
test("host acceptance 8 (simulated): the last uninstall removes unchanged canonical content", async () => {
    await withTemporaryProject("host-uninstall-last", async (root) => {
        ...
        assert.ok(actions.some((action) => action.action === "remove" && action.path === `${CANONICAL_ROOT_POSIX}/forge-api/SKILL.md`));
...
test("host acceptance 9 (simulated): modified canonical content is preserved on uninstall", async () => {
    await withTemporaryProject("host-modified-canonical", async (root) => {
        ...
        assert.ok(actions.some((action) => action.action === "preserve-modified" &&
        ...
        await assert.rejects(stat(join(root, ...CANONICAL_ROOT_POSIX.split("/"), "forge-security", "SKILL.md")), { code: "ENOENT" });
    });
});
...
test("host acceptance 12 (simulated): an interrupted installation resumes to a fully resolvable layout", async () => {
    await withTemporaryProject("host-interrupted", async (root) => {
        await assert.rejects(install(root, "claude,cursor", { global: false, dryRun: false, interruptAfter: 25 }), /interruption after 25 managed write/u);
...
test("host acceptance 14 (simulated): symlink and reparse-point protection covers host roots and the canonical root", async (t) => {
    await withTemporaryProject("host-symlink", async (root) => {
        ...
        await assert.rejects(install(root, "claude", { global: false, dryRun: false }), /symlinked install destination/u);
...
test("host acceptance 15: the packed npm tarball carries the canonical tree every host adapter points at", async () => {
    ...
    const packed = await runFile(process.execPath, [await resolveNpmCli(), "pack", "--dry-run", "--json", "--ignore-scripts"], PACKAGE_ROOT, 10 * 60_000);
    assert.equal(packed.exitCode, 0, packed.stderr);
    const entries = JSON.parse(packed.stdout)[0]
        ?.files;
    ...
    assert.equal(skills.length, EXPECTED_SKILLS);
    for (const skill of skills)
        assert.ok(paths.has(`${CANONICAL_ROOT_POSIX}/${skill}/SKILL.md`), `packed tarball is missing canonical content for ${skill}`);
    for (const hostRoot of HOSTS.map((host) => host.skillsRoot.join("/")))
        assert.ok(paths.has(`${hostRoot}/fullstack-forge/SKILL.md`), `packed tarball is missing the ${hostRoot} adapter`);
...
async function resolveNpmCli() {
    ...
    for (const value of candidates) {
        ...
        if (basename(candidate).toLowerCase() !== "npm-cli.js")
            continue;
        try {
...
//# sourceMappingURL=host-acceptance.test.js.map