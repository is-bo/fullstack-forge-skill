...
import { BUILD_PRODUCER_REGISTRY, executeBuildProducer, registeredBuildProducer } from "../src/build-producers.js";
...
async function pass(scriptName, criterion) {
...
test("high-tier security and migration criteria have exact registered producers", () => {
    for (const criterion of [
        ...
        assert.ok(BUILD_PRODUCER_REGISTRY.some((entry) => entry.criterion === criterion), criterion);
    assert.equal(new Set(BUILD_PRODUCER_REGISTRY.map((entry) => `${entry.script_name}\u0000${entry.criterion}`))
        .size, BUILD_PRODUCER_REGISTRY.length);
    assert.equal(new Set(BUILD_PRODUCER_REGISTRY.map((entry) => entry.id)).size, BUILD_PRODUCER_REGISTRY.length);
});
