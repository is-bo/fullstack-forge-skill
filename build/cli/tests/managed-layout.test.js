...
import { ADAPTER_MARKER, ADAPTER_SCHEMA_VERSION, CANONICAL_ROOT_POSIX, adapterPointer, extractFrontmatter, isAdapter, isVerbatimHostFile, readAdapterMarker, renderAdapter } from "../src/managed-layout.js";
const generator = (await import(pathToFileURL(join(PACKAGE_ROOT, "scripts", "lib", "managed-layout.mjs")).href));
const HOST_ROOTS = [
    ".agents/skills",
    ".claude/skills",
    ".cursor/skills",
...
test("installer and generator render byte-identical adapters for every host root", () => {
    ...
    for (const hostRoot of HOST_ROOTS) {
        ...
        assert.equal(pointer, `${"../".repeat(hostRoot.split("/").length + 1)}${CANONICAL_ROOT_POSIX}/forge-api/SKILL.md`);
