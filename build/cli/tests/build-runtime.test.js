...
import { casesFromRenderedCapture, deriveBuildRuntimeEvidence, planBuildRuntime } from "../src/build-runtime.js";
const SHA = "a".repeat(64);
function renderedCapture(observations = ["desktop", "tablet", "mobile"].map((name, index) => ({
    name,
...
test("runtime plan has a deterministic UI state matrix and only allowlisted actions", () => {
    ...
    assert.deepEqual(plan.cases[0]?.actions.map((action) => action.kind), [
        "navigate",
        "set-viewport",
        "capture-screenshot",
...
test("partial, unavailable, console, keyboard, overflow, and accessibility outcomes fail closed", () => {
    ...
    if (first === undefined)
        throw new Error("test plan unexpectedly empty");
    first.accessibility = "NOT_VERIFIED";
    assert.equal(deriveBuildRuntimeEvidence({
        plan,
        rendered,
        cases: unavailable,
        design_direction: { status: "PRESENT", follows_direction: true }
    })[0].status, "NOT_VERIFIED");
});
...
test("every planned state needs its own complete, one-to-one rendered capture", () => {
    ...
    const loadingCapture = { state: "loading", rendered: loading };
    const successCapture = { state: "success", rendered: success };
    const evidence = deriveBuildRuntimeEvidence({
        plan,
        rendered: loading,
...
test("fixed overflow and inaccessible custom-control observations fail Build runtime evidence", () => {
    ...
    if (observations === undefined)
        throw new Error("test capture unexpectedly lacks observations");
