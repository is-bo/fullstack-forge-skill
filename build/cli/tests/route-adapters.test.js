...
async function routesFor(name, files) {
    let routes = [];
...
test("Next.js App Router route handlers are detected", async () => {
    ...
    assert.ok(routes.every((route) => route.evidence.some((item) => item.includes("nextjs-app-router"))), "each record must name the adapter that produced it");
});
...
test("framework routes do not claim a visibility that middleware may control", async () => {
    ...
    assert.equal(route.visibility, "unknown", "an adapter that cannot see the middleware chain must not assert visibility");
...
test("a route named /login is not asserted public at high confidence", async () => {
    ...
    assert.ok(route !== undefined);
    assert.equal(route.confidence, "LOW", "name-based visibility is a heuristic and must be reported at low confidence");
    assert.ok(route.evidence.some((item) => item.includes("inferred from the route name")), "the heuristic must be disclosed in the evidence");
});
