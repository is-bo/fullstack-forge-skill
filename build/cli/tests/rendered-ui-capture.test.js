...
function fakeChromium(spec, recorder) {
    return {
        launch: () => {
            if (spec.launchError !== undefined)
            ...
            const page = {
                ...
                goto: async (target) => {
                    ...
                    if (viewportIndex === 0) {
                        ...
                        for (const message of spec.pageErrors ?? [])
                            pageErrorHandler?.(new Error(message));
                    }
                ...
                screenshot: async (options) => {
                    ...
                    if ((spec.skipWrite ?? []).includes(viewportIndex))
                        return undefined;
            ...
            return Promise.resolve({
                newPage: () => {
                    if (spec.newPageError !== undefined)
                        throw new Error(spec.newPageError);
                    return Promise.resolve(page);
                },
                close: () => {
                    recorder.closeCalls += 1;
                    if (spec.closeError !== undefined)
                        throw new Error(spec.closeError);
                    return Promise.resolve();
                }
            });
        }
...
async function capture(spec, overrides = {}) {
    let outcome;
...
test("offline mode aborts every non-loopback request before it reaches the network", async () => {
    ...
    const hostOf = (raw) => new URL(raw).hostname;
...
test("blocked request URLs are redacted before they are recorded", async () => {
    ...
    const { outcome } = await capture({ requests: [`https://api.example.com/data?token=${sentinel}#${sentinel}`] }, { offline: true });
...
test("online mode intercepts nothing and reaches every destination", async () => {
    ...
    assert.equal(record.reachedNetwork.filter((raw) => new URL(raw).hostname === "cdn.example.com").length, 3, "online mode reaches the remote destination unchanged");
    assert.equal(outcome.capture_status, "COMPLETE");
});
