 ...
 */
const GLOBAL_ADMINISTRATOR_ROLE = /super[_-]?admin|superadmin|platform[_-]?admin|global[_-]?admin|system[_-]?admin|instance[_-]?admin|site[_-]?admin|root[_-]?admin|\bsuperuser\b|\bsysadmin\b/iu;
/**
 ...
 */
const TENANT_ADMINISTRATOR_ROLE = /tenant[_-]?admin|org(?:anisation|anization)?[_-]?admin|workspace[_-]?admin|account[_-]?admin|team[_-]?admin|group[_-]?admin|company[_-]?admin|project[_-]?admin|clinic[_-]?admin|store[_-]?admin|school[_-]?admin|\bmembership\b|member[_-]?role/iu;
/**
 ...
 */
const AMBIGUOUS_ADMINISTRATOR_ROLE = /admin|\bstaff\b|\bmoderator\b|\boperator\b|\bsudo\b|\bsuperuser\b/iu;
/** Keys that make an object part of an owner or tenant partition rather than platform state. */
const OWNERSHIP_PARTITION_KEY = /\bowner\b|owner[_-]?id|owned[_-]?by|\buser[_-]?id\b|created[_-]?by|belongs[_-]?to|\bmembership\b|\bmember[_-]?id\b/iu;
/** Declarations that publish an explicit global-administrator role mapping. */
const GLOBAL_ROLE_DECLARATION = /\b(?:global|platform|super|system|instance)[_-]?admin(?:istrator)?[_-]?roles?\b[^=]{0,80}=([^;\n]{0,400}(?:\n[^;]{0,400})?)/giu;
 ...
 */
export function decideObjectAuthorization(input) {
    ...
    if (input.authority === "tenant")
        return {
            outcome: "missing",
            reason: "the gate proves a partition administrator, which does not establish authority over an object selected by a global identifier"
        };
    ...
    if (input.authority === "ambiguous")
        return {
            outcome: "unresolved",
            reason: "an administrative gate was observed, but the source never states whether that role spans every object of this resource"
        };
    if (input.partition === "partitioned")
        return {
            outcome: "unresolved",
            reason: "a platform administrator gate was proven, but the resource is owner- or tenant-partitioned and nothing proves the role spans every partition"
        };
    return {
        outcome: "administrative",
        reason: "a proven platform administrator acts on a resource with no observable owner or tenant partition"
    };
}
/**
 ...
 */
export function classifyAdministrativeAuthority(evidence, globalRoles) {
    const text = evidence.text;
    if (text.length === 0)
        return "none";
    const declaredGlobal = [...globalRoles].some((role) => new RegExp(`["'\`]${escapeRegExp(role)}["'\`]`, "u").test(text));
    const administrative = declaredGlobal ||
    ...
    if (TENANT_ADMINISTRATOR_ROLE.test(text))
        return "tenant";
    if (declaredGlobal || GLOBAL_ADMINISTRATOR_ROLE.test(text))
        return "global";
    return "ambiguous";
}
/** Combines authority evidence from several gates covering the same sink. */
export function strongerAuthority(left, right) {
 ...
 */
export function collectGlobalAdministratorRoles(sources) {
    const roles = new Set();
    for (const source of sources) {
        GLOBAL_ROLE_DECLARATION.lastIndex = 0;
        for (let match = GLOBAL_ROLE_DECLARATION.exec(source.content); match !== null; match = GLOBAL_ROLE_DECLARATION.exec(source.content)) {
            const initializer = match[1] ?? "";
            ROLE_LITERAL.lastIndex = 0;
            for (let literal = ROLE_LITERAL.exec(initializer); literal !== null; literal = ROLE_LITERAL.exec(initializer)) {
 ...
 */
export function classifyResourcePartition(context, tenantKeyPattern) {
    if (OWNERSHIP_PARTITION_KEY.test(context))
        return "partitioned";
