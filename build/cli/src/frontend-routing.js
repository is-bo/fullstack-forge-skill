...
const BACKEND_ONLY = /\b(?:backend|server library|database|schema|migration|prisma|sql|orm|api pagination|api page size|form parser|request parser|service layer|repository layer)\b/iu;
...
export function routeFrontendRequest(request, area, evidence = {}) {
    ...
    const reasons = [];
    const supportingEvidence = strongFrontend || strongUi || strongUx || interfaceCrossCuttingEvidence || repositorySupport;
