...
import { assertNoSymlinkPath, assertSafeRelative, canonicalDirectory, resolveInside, sha256, utcNow, workingTreeRevision } from "./utils.js";
const SAFE_DEFINITIONS = [
    {
        fix_id: "FF-FIX-ENV-PLACEHOLDER-001",
        supported_finding_pattern: "^FF-ENV-TEMPLATE-001$",
        section: "security",
        description: "Replace actual-looking values in environment examples with explicit placeholders.",
        risk: "safe",
        preconditions: [
        ...
        expected_original_state: "A KEY=value line at the finding location contains a non-placeholder credential-like value.",
        planned_edits: "Replace only the value token with <REPLACE_WITH_SECRET>.",
        verification: "Reparse and re-run structured-config-safety; provider-side rotation remains manual.",
        rollback: "Restore the exact original line from version control; the secret value is not copied into a rollback artifact.",
    ...
    },
    {
        ...
        expected_original_state: "A literal target=_blank anchor lacks one or both required rel tokens.",
    ...
    },
    {
        ...
        expected_original_state: "The existing global rule has a headers array without X-Content-Type-Options.",
        planned_edits: "Append { key: X-Content-Type-Options, value: nosniff } and serialize deterministically.",
        verification: "Parse vercel.json and re-run structured-config-safety.",
        rollback: "Remove the exact appended header object or restore the original file from version control.",
...
const RISKY_PATTERNS = [
    {
        ...
        reason: "Authorization and tenant semantics require an explicitly authorized risky-change mechanism."
    },
    {
        pattern: /^FF-UPLOAD-/u,
        reason: "Upload limits, type policy, quarantine, and scanner behavior require existing product policy or explicit approval."
    },
    {
...
export const FIX_REGISTRY = [
    ...SAFE_DEFINITIONS.map(publicRegistryEntry),
    {
        ...
        section: "multiple",
        description: "Approval-bound product, identity, tenant, upload, AI, payment, or integration change.",
        risk: "risky",
...
export async function executeFixes(rootInput, section, options) {
    ...
    const scoped = report.findings.filter((finding) => (section === "all" || finding.section === section) &&
    ...
    const regressionCommand = (await detectProjectCommands(root)).find((command) => command.name === "test");
    ...
    const regression = regressionCommand === undefined
        ? undefined
...
function hasPreviouslyResolvedSafeFix(report, section) {
    return report.findings.some((finding) => (section === "all" || finding.section === section) &&
...
function planEnvironmentPlaceholder(finding, path, content) {
    ...
    const targetLines = new Set(finding.location
    ...
    if (!changed)
        throw new Error(`No confirmed environment finding location was found in ${path}.`);
    return lines.join("");
}
function planBlankRel(finding, path, content) {
    ...
    if (diagnostics.length > 0)
        throw new Error(`Refusing to transform invalid JSX: ${path}`);
    const targetLines = new Set(finding.location
        .filter((location) => location.path === path)
        .flatMap((location) => location.line ?? []));
    const edits = [];
    walk(sourceFile, (node) => {
        if (!ts.isJsxOpeningElement(node) && !ts.isJsxSelfClosingElement(node))
            return;
        if (node.tagName.getText(sourceFile).toLowerCase() !== "a")
            return;
...
function planVercelNosniff(_finding, path, content) {
    ...
    for (const rule of parsed.headers) {
        if (!isRecord(rule) || typeof rule.source !== "string" || !Array.isArray(rule.headers))
            continue;
        if (!["/(.*)", "/(.*)?", "/*", "/:path*"].includes(rule.source))
            continue;
        const present = rule.headers.some((header) => isRecord(header) &&
    ...
    if (!changed)
        throw new Error(`No eligible global Vercel header rule remains in ${path}.`);
    const newline = content.includes("\r\n") ? "\r\n" : "\n";
    return `${JSON.stringify(parsed, null, detectIndent(content)).replaceAll("\n", newline)}${newline}`;
}
async function writeIfUnchanged(root, operation) {
    ...
    try {
        ...
        if (!currentInfo.isFile())
            throw new Error(`Refusing non-file fix handle: ${operation.path}`);
...
async function verifyAppliedFixes(root, operations) {
...
async function rollbackWrites(root, operations) {
...
function looksLikeCredential(value) {
    ...
    return (normalized.length >= 12 &&
        ...
        !/^(?:your[-_ ]|example|sample|test|dummy|fixture|placeholder|changeme|xxx|<|\$\{|\*+)/iu.test(normalized));
}
function walk(node, callback) {
    callback(node);
    node.forEachChild((child) => walk(child, callback));
}
function jsxLiteral(node, name) {
