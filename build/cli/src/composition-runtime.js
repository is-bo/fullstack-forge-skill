...
export async function resolveRuntimeComposition(root, modules, evidence, runtimeRootOverride) {
    ...
    for (const module of modules) {
        ...
        const contractPath = join(".fullstack-forge", "skills", "fullstack-forge", result.selected[0]?.runtimePath ?? "").replaceAll("\\", "/");
...
export async function writeCompositionArtifact(root, results) {
    ...
    await writeFile(path, `${JSON.stringify({ schemaVersion: 1, compositions: results }, null, 2)}\n`, "utf8");
    return path;
}
