import { ANALYZER_SUPPORT_STATUSES, MODULE_APPLICABILITY_STATUSES, MODULE_CAPABILITY_STATUSES, MODULE_SELECTION_STATUSES, NETWORK_POLICIES, PLANNED_CHECK_STATUSES, RUNTIME_EVIDENCE_STATUSES, TOOL_OWNERSHIPS, TOOL_TRUST_LEVELS, TOOL_VERSION_SOURCES } from "./types.js";
/**
 ...
 */
export function createPlannedCheck(input) {
...
export function appendPlannedCheck(ledger, check) {
    assertPlannedChecks([check]);
 ...
 */
export function recordExecutedCheck(ledger, checkId, detail = {}) {
    const current = findCheck(ledger, checkId);
    if (current.status === "BLOCKED" || current.status === "NOT_APPLICABLE")
        throw new Error(`Check '${checkId}' is recorded ${current.status}; it cannot be re-recorded as RUN.`);
...
export function recordBlockedCheck(ledger, checkId, reason) {
 ...
 */
export function appendRuntimeEvidence(ledger, evidence) {
...
export function appendModuleDecision(ledger, decision) {
...
export function appendToolRecord(ledger, tool) {
    assertToolRecords([tool]);
    return mergeById(ledger, tool, (entry) => entry.tool_id, mergeToolRecord);
}
function mergeToolRecord(current, next) {
    return { ...current, ...next, limitations: union(current.limitations, next.limitations) };
}
function mergePlannedCheck(current, next) {
    if (current.status !== next.status && next.status === "RUN")
        throw new Error(`Check '${current.check_id}' is recorded ${current.status}; it cannot be re-recorded as RUN.`);
    return {
        ...current,
        ...next,
...
function mergeRuntimeEvidence(current, next) {
    if (next.status === "PASS" && current.status !== "PASS")
        throw new Error(`Runtime evidence '${current.evidence_id}' is recorded ${current.status}; it cannot be re-recorded as PASS.`);
    return {
        ...current,
        status: next.status,
...
function mergeModuleDecision(current, next) {
    return {
        ...current,
...
function mergeById(ledger, value, key, merge) {
    const identity = key(value);
    const index = ledger.findIndex((entry) => key(entry) === identity);
    if (index === -1)
        return [...ledger, structuredClone(value)];
...
function findCheck(ledger, checkId) {
    const current = ledger.find((entry) => entry.check_id === checkId);
    if (current === undefined)
        throw new Error(`No planned check '${checkId}' exists in the ledger.`);
    return current;
}
function replaceCheck(ledger, checkId, next) {
    assertPlannedChecks([next]);
    return ledger.map((entry) => entry.check_id === checkId ? structuredClone(next) : structuredClone(entry));
}
function union(left, right) {
    return [...new Set([...left, ...right])];
}
export function assertPlannedChecks(values) {
    const errors = [];
    const seen = new Set();
    for (const [index, value] of values.entries()) {
        ...
        else if (seen.has(value.check_id))
            errors.push(`[${index}] duplicate check_id`);
        else
            seen.add(value.check_id);
        for (const field of ["module", "source"])
            if (typeof value[field] !== "string" || value[field].trim().length === 0)
                errors.push(`[${index}] ${field} must be a non-empty string`);
        if (!PLANNED_CHECK_STATUSES.includes(value.status))
            errors.push(`[${index}] invalid planned check status`);
        else if (value.status !== "RUN" &&
        ...
        if (!NETWORK_POLICIES.includes(value.network_policy))
            errors.push(`[${index}] invalid network_policy`);
    }
    if (errors.length > 0)
        throw new Error(`Invalid planned checks:\n${errors.join("\n")}`);
}
export function assertRuntimeEvidence(values) {
    const errors = [];
    const seen = new Set();
    for (const [index, value] of values.entries()) {
        if (!isRecord(value)) {
        ...
        else if (seen.has(value.evidence_id))
            errors.push(`[${index}] duplicate evidence_id`);
        else
            seen.add(value.evidence_id);
        for (const field of ["evidence_type", "revision"])
            if (typeof value[field] !== "string" || value[field].trim().length === 0)
                errors.push(`[${index}] ${field} must be a non-empty string`);
        if (!RUNTIME_EVIDENCE_STATUSES.includes(value.status))
            errors.push(`[${index}] invalid runtime evidence status`);
        if (!Array.isArray(value.artifact_paths) ||
...
export function assertModuleDecisions(values) {
    const errors = [];
    const seen = new Set();
    for (const [index, value] of values.entries()) {
        if (!isRecord(value)) {
        ...
        else if (seen.has(value.module))
            errors.push(`[${index}] duplicate module decision`);
        else
            seen.add(value.module);
        if (!MODULE_CAPABILITY_STATUSES.includes(value.capability_status))
            errors.push(`[${index}] invalid capability_status`);
        if (value.risk_status !== undefined &&
            !MODULE_CAPABILITY_STATUSES.includes(value.risk_status))
            errors.push(`[${index}] invalid risk_status`);
        if (value.control_status !== undefined &&
            !MODULE_CAPABILITY_STATUSES.includes(value.control_status))
            errors.push(`[${index}] invalid control_status`);
        if (value.applicability_status !== undefined &&
            !MODULE_APPLICABILITY_STATUSES.includes(value.applicability_status))
            errors.push(`[${index}] invalid applicability_status`);
        if (value.analyzer_support !== undefined &&
            !ANALYZER_SUPPORT_STATUSES.includes(value.analyzer_support))
            errors.push(`[${index}] invalid analyzer_support`);
        if (!MODULE_SELECTION_STATUSES.includes(value.selection_status))
    ...
    if (errors.length > 0)
        throw new Error(`Invalid module decisions:\n${errors.join("\n")}`);
}
export function assertToolRecords(values) {
    const errors = [];
    const seen = new Set();
    for (const [index, value] of values.entries()) {
        if (!isRecord(value)) {
        ...
        else if (seen.has(value.tool_id))
            errors.push(`[${index}] duplicate tool_id`);
        else
            seen.add(value.tool_id);
        for (const field of ["name", "version"])
            if (typeof value[field] !== "string" || value[field].trim().length === 0)
                errors.push(`[${index}] ${field} must be a non-empty string`);
        if (!TOOL_OWNERSHIPS.includes(value.ownership))
            errors.push(`[${index}] invalid ownership`);
        if (!TOOL_TRUST_LEVELS.includes(value.trust))
            errors.push(`[${index}] invalid trust level`);
        if (!TOOL_VERSION_SOURCES.includes(value.version_source))
