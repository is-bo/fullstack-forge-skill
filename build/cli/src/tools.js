...
export async function validateBundledSkills() {
    ...
    const rawCriteria = JSON.parse(await readFile(join(PACKAGE_ROOT, "config", "module-criteria.json"), "utf8"));
    const criteriaBySlug = typeof rawCriteria === "object" && rawCriteria !== null && !Array.isArray(rawCriteria)
        ? rawCriteria
        : {};
    ...
    for (const workflowCommand of ["forge", "forge-new", "forge-feature"]) {
        paths.push(join(PACKAGE_ROOT, "src", "fullstack-forge", "commands", workflowCommand, "SKILL.md"));
    }
