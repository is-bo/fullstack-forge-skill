...
export async function runShipGates(root, profile, previous, commands, allowRun) {
    ...
    for (const definition of FORGE_GATE_REGISTRY) {
        ...
        if (definition.gate_id === "FF-GATE-DEPENDENCIES") {
            const dependencyEvidence = previous?.findings.filter((finding) => finding.section === "supply-chain") ?? [];
            gates.push({
                gate_id: definition.gate_id,
                name: definition.name,
                category: definition.category,
                ...
                status: dependencyEvidence.some((finding) => finding.status === "FAIL")
                    ...
                    : dependencyEvidence.some((finding) => finding.status === "PASS")
                        ? "PASS"
                        : "NOT_VERIFIED",
                evidence: dependencyEvidence.length > 0
...
function gate(gateId, name, category, command) {
    return {
        gate_id: gateId,
        name,
        category,
