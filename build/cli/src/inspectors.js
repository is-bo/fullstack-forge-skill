...
const TOOL_PATTERNS = {
    ...
    "inspect-auth-boundaries": [
        {
            ...
            regex: /\b(?:authenticate|requireAuth|authMiddleware|getServerSession|verifyToken|session)\b/giu,
    ...
    "inspect-authorization": [
        {
            ...
            regex: /\b(?:authorize|permission|policy|canAccess|requireRole|isAdmin|tenantId|organizationId)\b/giu,
    ...
    "inspect-database-schema": [
        {
            category: "schema",
            regex: /\b(?:CREATE\s+TABLE|ALTER\s+TABLE|model\s+\w+\s*\{|Schema\s*\(|createTable|addColumn)\b/giu,
 ...
 */
export async function inspectSection(section, root, profile, scope, repositoryInventory, decision) {
    ...
    for (const file of files) {
        ...
        for (const [index, line] of content.split(/\r?\n/u).entries()) {
            ...
            if (looksLikeSecret(value)) {
                findings.push(finding("ENV", findings.length + 1, "security", "Template contains an actual-looking secret value", "HIGH", "MEDIUM", "FAIL", toPosix(relative(root, file)), index + 1, "A non-placeholder credential-like value appears in an environment template; the value is intentionally redacted.", "Published templates can disclose credentials.", "Replace it with a clear placeholder and rotate the value if it was ever valid.", true, ["Re-scan templates and confirm the credential is revoked where applicable"], ["OWASP Secrets Management Cheat Sheet"]));
            }
        }
    }
...
async function scanSecretPatterns(root, scope, repositoryInventory) {
    ...
    const patterns = [
        ...
        },
        {
            ...
            regex: /\b(?:[a-z0-9_]*(?:api[_-]?key|secret|token|password))\s*[:=]\s*["']([^"'\r\n]{12,})["']/giu,
            severity: "HIGH",
            confidence: "LOW"
        }
    ];
    ...
    for (const file of selection.files) {
        ...
        for (const pattern of patterns) {
            ...
            for (const match of content.matchAll(pattern.regex)) {
                ...
                findings.push(finding("SECRET", findings.length + 1, "security", `Potential ${pattern.name} in source`, pattern.severity, pattern.confidence, "FAIL", path, line, `Pattern matched at ${path}:${line}; value redacted.`, "A committed credential may enable unauthorized access.", "Triage without exposing the value, remove it from distributable content, and rotate if valid.", false, [
    ...
    for (const relRoot of roots) {
        ...
        for (const entry of entries) {
            ...
            if (!/^---\r?\n[\s\S]*?\r?\n---\r?\n/u.test(content) ||
                ...
                findings.push(finding("SKILL", findings.length + 1, "docs", "Agent Skill frontmatter is incomplete", "MEDIUM", "HIGH", "FAIL", path, 1, "SKILL.md lacks a valid frontmatter block with name and description.", "Agents may not discover or activate the skill.", "Add standards-compliant name and description frontmatter.", true, ["Run validate-skill"], ["Agent Skills Specification"]));
            }
        }
    }
...
function isCiFile(path) {
    return /^(?:\.github\/workflows\/.*\.ya?ml|\.gitlab-ci\.yml|\.circleci\/config\.yml|azure-pipelines\.yml|Jenkinsfile)$/iu.test(path);
}
 ...
 */
function resolveApplicability(section, profile, decision) {
    const resolved = decision ?? decideModules({ candidates: [section], profile, explicit: false })[0];
    if (resolved === undefined)
        return { status: "APPLICABLE_UNPROVEN", evidence: [] };
    ...
    const status = resolved.applicability_status ??
...
function notApplicableFinding(section, evidence) {
    return {
...
function finding(prefix, number, section, title, severity, confidence, status, path, line, evidence, impact, recommendation, safeFix, verification, standards) {
    return {
