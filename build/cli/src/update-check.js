...
export const RELEASES_API_URL = "https://api.github.com/repos/is-bo/fullstack-forge-skill/releases/latest";
const LEGACY_RELEASE_ARCHIVE_ROOT = "https://codeload.github.com/is-bo/fullstack-forge-skill/tar.gz/refs/tags";
 ...
 */
export function parseReleaseChannel(payload) {
    let parsed;
    try {
    ...
    if (!isRecord(parsed))
        throw new Error("GitHub Release channel returned an invalid object");
    if (parsed.draft !== false)
        throw new Error("GitHub Release channel returned a draft release");
    if (parsed.prerelease !== false)
        throw new Error("GitHub Release channel returned a prerelease");
...
export function publicReleaseArchive(versionText) {
...
export async function checkUpdateAvailability(root, offline, currentVersion = VERSION, reader = readPublicReleaseChannel) {
    if (offline)
        return {
            status: "WARNING",
...
function requiredPlatformAssets(version) {
    const tag = `v${version}`;
    return [
...
function assertReleaseAssetUrl(urlText, tag, name, index) {
    let url;
    try {
        url = new URL(urlText);
    }
    catch {
        throw new Error(`GitHub Release channel returned an invalid asset download URL at index ${index}`);
    }
...
function readPublicReleaseChannel(request) {
    return new Promise((resolvePromise, reject) => {
        ...
        const networkRequest = get(request.url, {
            headers: {
        ...
        networkRequest.setTimeout(request.timeoutMs, () => networkRequest.destroy(new Error("GitHub Release channel request timed out")));
...
function requiredVersion(value) {
    ...
    if (version === undefined)
        throw new Error("Internal release-channel version policy is invalid");
    return version;
}
