...
export function migrateReport(value) {
    ...
    const compositions = take("compositions");
    const base = source;
    const migrated = {
        ...base,
...
function assertCompositionResults(results) {
    ...
    for (const result of results) {
        ...
        if (typeof result.module !== "string" ||
        ...
        if (result.eager !== undefined || result.deferred !== undefined) {
            ...
            const selectedKeys = new Set(result.selected.map((source) => `${source.tier}\u0000${source.provider}\u0000${source.skill}`));
...
export function renderMarkdown(report) {
    ...
    const compositions = report.compositions.map(renderComposition).join("\n\n") ||
...
function renderComposition(result) {
    const selected = result.selected
        .map((source) => `  - **${source.tier}** \`${source.runtimePath}\` (${source.provider}/${source.skill}): ${source.reason}`)
        .join("\n");
