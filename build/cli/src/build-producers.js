...
const DISCIPLINE_PRODUCERS = DISCIPLINE_SCRIPTS.map((discipline) => producer(`test:${discipline}`, `discipline:${discipline}`, discipline, SECURITY_DISCIPLINES.has(discipline), SECURITY_DISCIPLINES.has(discipline)));
export const BUILD_PRODUCER_REGISTRY = uniqueProducers([
    ...
    producer("test:authorization-negative", "authorization-negative-tests", "authorization", true, true),
...
export const BUILD_INTERNAL_PRODUCER_REGISTRY = [
    ...
    ...DISCIPLINE_SCRIPTS.map((discipline) => internalProducer(`fullstack-forge/build-applicability/${discipline}`, `discipline:${discipline}`, discipline, SECURITY_DISCIPLINES.has(discipline), SECURITY_DISCIPLINES.has(discipline)))
];
export const BUILD_UNAVAILABLE_PRODUCER = "fullstack-forge/build-unavailable";
export function registeredBuildProducer(scriptName, criterion) {
    return BUILD_PRODUCER_REGISTRY.find((entry) => entry.script_name === scriptName && (criterion === undefined || entry.criterion === criterion));
}
export function registeredBuildProducerById(producerId, criterion) {
    return [...BUILD_PRODUCER_REGISTRY, ...BUILD_INTERNAL_PRODUCER_REGISTRY].find((entry) => entry.id === producerId && entry.criterion === criterion);
}
/** Validates code-owned producer identity and status semantics; it never treats prose as proof. */
export function buildProducerContractProblems(claim) {
    const problems = [];
    ...
    if (claim.security_control !== producer.security_control)
        problems.push("Build producer security-control classification does not match its registry entry.");
 ...
 */
export async function executeBuildProducer(input) {
    const now = input.now ?? utcNow;
    const recordedAt = now();
    const expiresAt = new Date(Date.parse(recordedAt) + BUILD_PRODUCER_EXPIRY_MS).toISOString();
    const command = input.command;
    const matched = command === undefined ? undefined : registeredBuildProducer(command.name, input.criterion);
    const base = (producer, status, limitations) => observation(producer, command, status, input.input_manifest, recordedAt, expiresAt, limitations);
    if (command === undefined)
        return unavailableObservation(input.criterion, input.input_manifest, recordedAt, expiresAt, "No detected command was supplied for this Build criterion.");
    if (matched === undefined)
        return unavailableObservation(input.criterion, input.input_manifest, recordedAt, expiresAt, `Detected script '${command.name}' has no registered Build producer for '${input.criterion}'.`);
    ...
    const manifestProblem = validateInputManifest(input.input_manifest, input.input_manifest_complete);
    if (manifestProblem !== undefined)
        return base(matched, "NOT_VERIFIED", [manifestProblem]);
    if (!input.allow_run)
    ...
    if (!decision.permitted)
        return base(matched, "BLOCKED", [decision.reason]);
    const startedAt = now();
...
function producer(scriptName, criterion, discipline, securityControl = false, nonWaivable = false) {
    return {
...
function internalProducer(id, criterion, discipline, securityControl = false, nonWaivable = false) {
    return {
        id,
        version: BUILD_PRODUCER_VERSION,
        contract: BUILD_PRODUCER_CONTRACT,
...
function uniqueProducers(entries) {
    const unique = new Map();
    for (const entry of entries)
        unique.set(`${entry.script_name}\0${entry.criterion}`, entry);
    return [...unique.values()];
}
function observation(producer, command, status, inputManifest, recordedAt, expiresAt, limitations) {
    return {
        domain: "Build",
...
function unavailableObservation(criterion, inputManifest, recordedAt, expiresAt, limitation) {
    return {
        domain: "Build",
...
function isDetectedCommand(command) {
...
function validateInputManifest(inputManifest, complete) {
    if (!complete)
        return "The caller did not attest that the input manifest covers every relevant input file.";
    if (inputManifest.length === 0)
        return "A Build producer requires a non-empty input manifest.";
    const paths = new Set();
    try {
        for (const input of inputManifest) {
            ...
            if (paths.has(input.path))
                return `Input manifest path '${input.path}' is duplicated.`;
...
async function defaultBuildProducerRunner(command, root) {
    return runFile(command.executable, command.args, root);
}
function errorMessage(error) {
    return error instanceof Error ? error.message : String(error);
}
//# sourceMappingURL=build-producers.js.map