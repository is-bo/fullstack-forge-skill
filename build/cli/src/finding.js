...
export function validateFinding(value) {
    ...
    if ("evidence_snapshot" in value) {
        if (!Array.isArray(value.evidence_snapshot))
            errors.push("evidence_snapshot must be an array");
        else {
...
export function assertAgentFindings(values) {
    ...
    const errors = values.flatMap((value, index) => value.producer === "agent-reviewed-source" ||
        value.producer === "agent-rendered-review" ||
        value.producer === "agent-runtime-verification"
        ? []
        : [
...
function validRenderedEvidence(value, errors) {
    ...
    for (const [index, item] of value.entries()) {
        ...
        for (const field of ["artifact_path", "url", "state"])
...
function isVerificationAction(value) {
    if (!isRecord(value) || typeof value.type !== "string")
        return false;
    if (value.type === "analyzer") {
        const instanceValid = !("instance_id" in value) ||
            (typeof value.instance_id === "string" &&
                /^FF-[A-Z0-9-]+-[0-9]{3,}:[a-f0-9]{8,}$/u.test(value.instance_id) &&
                typeof value.finding_id === "string" &&
                value.instance_id.startsWith(`${value.finding_id}:`));
        const scopeValid = !("scope_paths" in value) ||
            (Array.isArray(value.scope_paths) &&
                value.scope_paths.every((path) => typeof path === "string" && isSafeEvidencePath(path)));
        return (typeof value.analyzer_id === "string" &&
            typeof value.finding_id === "string" &&
