 ...
 */
export const CANONICAL_ROOT_SEGMENTS = [".fullstack-forge", "skills"];
...
export function adapterPointer(hostSkillsRootPosix, skill) {
    ...
    if (segments.length === 0)
        throw new Error("Host skills root must not be empty");
...
export function extractFrontmatter(text, label) {
    ...
    if (end === -1)
        throw new Error(`Canonical skill ${label} has an unterminated frontmatter block`);
...
export function readAdapterMarker(text) {
    const match = new RegExp(`<!-- ${ADAPTER_MARKER} v(\\d+) skill=([^\\s]+) canonical=([^\\s]+) -->`, "u").exec(text);
    if (match === null)
        return undefined;
