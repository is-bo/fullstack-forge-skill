 ...
 */
export async function executeProjectCommand(root, command, options) {
    ...
    if (!options.allowRun) {
        return {
            ...
            ledger: {
                ...
                reason: "Execution requires explicit --allow-run after reviewing the local project script definition."
            }
        };
    }
    if (options.dryRun) {
    ...
    const result = await runFile(command.executable, command.args, root, options.timeoutMs ?? 10 * 60_000);
    const execution = {
