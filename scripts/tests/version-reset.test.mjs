...
test("release candidate version is consistently 0.2.1 without a premature publication claim", async () => {
  ...
  assert.match(constants, /VERSION = "0\.2\.1"/u);
  assert.match(
    readme,
    /npm install --save-dev "https:\/\/codeload\.github\.com\/is-bo\/fullstack-forge-skill\/tar\.gz\/refs\/tags\/v0\.2\.1"/u
  );
  ...
  assert.doesNotMatch(readme, /v0\.2\.1` is the current supported public release/iu);
});
