...
test("release candidate version is consistently 0.3.0 without a premature publication claim", async () => {
  ...
  const verification = await readFile(
    join(projectRoot, "docs", "RELEASE_VERIFICATION_v0.3.0.md"),
    "utf8"
  );
  ...
  assert.match(constants, /VERSION = "0\.3\.0"/u);
  assert.match(
    readme,
    /npm install --save-dev "https:\/\/github\.com\/is-bo\/fullstack-forge-skill\/releases\/download\/v0\.3\.0\/fullstack-forge-skill-v0\.3\.0\.tgz"/u
  );
  assert.match(readme, /upstream-powered/iu);
  assert.match(readme, /v0\.3\.0.*candidate/isu);
  ...
  assert.match(release, /`v0\.3\.0` is\s+the\s+current candidate/iu);
  ...
  assert.doesNotMatch(readme, /v0\.3\.0.*current supported public release/iu);
  ...
  assert.match(verification, /v0\.3\.0 is unpublished/iu);
});

test("release documents preserve historical evidence and add the v0.3.0 candidate", async () => {
