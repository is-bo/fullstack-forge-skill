...
export function decideModules(input) {
    ...
    for (const section of input.candidates) {
        ...
        if (input.explicit) {
            ...
            selection = "NOT_REQUESTED";
            reasons.push("The module was not loaded automatically because applicability is unproven. It remains available for explicit invocation, and the missing proof is recorded as NOT_VERIFIED.");
        }
 ...
 */
export function resolveImport(importer, request, files) {
...
function moduleImpact(profile, changed, reasons) {
    const impactReasons = new Map();
    const addImpact = (section, reason) => {
        const current = impactReasons.get(section) ?? new Set();
        current.add(reason);
    ...
    const affectedPaths = new Set([...new Set([...changed.keys(), ...reasons.keys()])]
    ...
    for (const assessment of profile.capability_assessments ?? []) {
        ...
        for (const evidence of assessment.evidence) {
            if (!Number.isFinite(evidence.activation_weight) || evidence.activation_weight <= 0)
                continue;
            ...
            for (const section of sections)
                addImpact(section, `affected path ${path} has activating '${assessment.capability}' capability evidence in workspace '${workspace}'`);
        }
    }
    for (const evidence of profile.risk_evidence ?? []) {
        const path = safeEvidencePath(evidence.path);
        if (path === undefined || !affectedPaths.has(path))
            continue;
    ...
    for (const path of affectedPaths) {
        if (FRONTEND_EXTENSIONS.has(extname(path).toLowerCase()))
            for (const section of ["accessibility", "frontend", "ui", "ux"])
                addImpact(section, `affected frontend source path ${path}`);
        const tokens = new Set(path
            .toLowerCase()
            .split("/")
...
function capabilityModules(capability) {
    const output = [];
    for (const [section, required] of Object.entries(SECTION_CAPABILITY))
...
function safeEvidenceRoot(value) {
