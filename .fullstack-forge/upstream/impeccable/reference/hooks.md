...
<!-- fullstack-forge: upstream installation instruction removed (impeccable) -->
...
| `on` | Set `enabled: true` in `.fullstack-forge/ui/config.json`, record local hook consent as accepted, and install/repair provider hook manifests when the skill is installed. |
| `off` | Set `enabled: false` in `.fullstack-forge/ui/config.json`. |
...
| `ignore-value <id> <value> [--shared] [--reason "..."]` | Append a rule/value suppression to shared `.fullstack-forge/ui/config.json`. |
| `ignore-value <id> <value> --local [--reason "..."]` | Append a private rule/value suppression to `.fullstack-forge/ui/config.local.json`. |
...
2. Invoke the admin script and pass the user's output through verbatim:
   ...
   ```bash
   node .fullstack-forge/upstream/impeccable/scripts/hook-admin.mjs <action> [args...]
   ```

3. If `<action>` is `off`, follow up with a one-line note: "Done. New edits will not trigger the design hook in this project until you run `$forge ui hooks on`."
4. If `<action>` is `on`, follow up with: "Done. The design hook will fire after the next Edit/Write/MultiEdit on a UI file."
5. If `<action>` is `ignore-value`, `ignore-file`, or `ignore-rule`, just print the script output. The default scope is shared `.fullstack-forge/ui/config.json`; add `--local` only when the user explicitly asks for a private exception.
...
- If the finding line shows an exact `ignore-value` command, run that command. This writes shared `.fullstack-forge/ui/config.json` by default.
...
```bash
node .fullstack-forge/upstream/impeccable/scripts/hook-admin.mjs ignore-value overused-font Inter --shared --reason "User confirmed Inter is intentional"
```

Example intentional motion exception:

```bash
node .fullstack-forge/upstream/impeccable/scripts/hook-admin.mjs ignore-value bounce-easing bounce-ball --shared --reason "User confirmed ball bounce animation is intentional"
```

Example whole-rule font exception:

```bash
node .fullstack-forge/upstream/impeccable/scripts/hook-admin.mjs ignore-rule overused-font --all-values --reason "User asked to ignore overused fonts generally"
```
...
```bash
node .fullstack-forge/upstream/impeccable/scripts/hook-admin.mjs ignore-value design-system-font-size "*" --file "src/overlay/widget.js" --reason "Injected widget builds its own type scale; DESIGN.md's ramp describes the site"
```

Example whole-file exception, for a file that is out of scope entirely:

```bash
node .fullstack-forge/upstream/impeccable/scripts/hook-admin.mjs ignore-file "src/legacy/Card.tsx"
```

## Constraints

- Never modify `.fullstack-forge/ui/config.json` or `.fullstack-forge/ui/config.local.json` by hand from this command. Always go through `hook-admin.mjs` so writes stay validated and the file shape stays consistent. One exception: `detector.extensions` has no admin action, so when the user asks to cover a template stack, edit that one field in `.fullstack-forge/ui/config.json` directly and leave the rest of the file untouched.
...
- If `.fullstack-forge/ui/config.json` or `.fullstack-forge/ui/config.local.json` is unreadable or malformed, the hook ignores that file and uses the remaining valid config/defaults. `hook-admin.mjs status` will show malformed files as ignored.
- If the user asks to "disable the hook" globally, lead with `$forge ui hooks off` (persistent for this project; writes `hook.enabled: false` to config). The legacy `IMPECCABLE_HOOK_DISABLED=1` env var also works as a one-shot override that follows the shell.
