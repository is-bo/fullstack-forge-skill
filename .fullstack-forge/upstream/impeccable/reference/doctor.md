Report and repair drift between this project's Impeccable artifacts and what the installed version reads: PRODUCT.md, DESIGN.md and its `.fullstack-forge/ui/design.json` sidecar, `.fullstack-forge/ui/config.json`, persisted surface briefs, and the design hook.
...
```
node .fullstack-forge/upstream/impeccable/scripts/doctor.mjs --json
```
...
- **`auto`** carries no decision. Run `node .fullstack-forge/upstream/impeccable/scripts/doctor.mjs --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
