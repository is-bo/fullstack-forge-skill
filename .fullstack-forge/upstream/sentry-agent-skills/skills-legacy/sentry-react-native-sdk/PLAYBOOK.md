...
-->
> All Skills _(unavailable upstream reference omitted)_ > SDK Setup _(unavailable upstream reference omitted)_ > React Native SDK
...
Source maps and debug symbols are what transform minified stack traces into readable ones. When set up correctly, Sentry shows you the exact line of your source code that threw. The generic auth-token and CI setup lives in `sentry-source-maps` _(unavailable upstream reference omitted)_; the React Native-specific upload mechanics are below.
