...
-->
> All Skills _(unavailable upstream reference omitted)_ > SDK Setup _(unavailable upstream reference omitted)_ > Next.js SDK
...
`SENTRY_AUTH_TOKEN` is a build-time secret, distinct from the DSN. For creating the token, wiring it into CI, and troubleshooting minified traces, see `sentry-source-maps` _(unavailable upstream reference omitted)_.
