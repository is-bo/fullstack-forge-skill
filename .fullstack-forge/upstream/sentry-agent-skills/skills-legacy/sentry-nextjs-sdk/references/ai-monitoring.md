...
### Per-Call Telemetry (Approval Required)

Only after the user explicitly approves Sentry telemetry and the prompt/output data boundary, pass `experimental_telemetry: { isEnabled: true }` to the specific AI SDK calls they approved:
...
If your `tracesSampleRate` is below 1.0, you may be losing entire agent runs. See the AI sampling guide _(unavailable upstream reference omitted)_ for `tracesSampler` patterns that keep 100% of gen_ai-related transactions while sampling other traffic at a lower rate.
