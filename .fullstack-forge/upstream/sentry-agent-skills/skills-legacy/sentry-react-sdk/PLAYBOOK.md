...
-->
> All Skills _(unavailable upstream reference omitted)_ > SDK Setup _(unavailable upstream reference omitted)_ > React SDK
...
`SENTRY_ORG` / `SENTRY_PROJECT` / `SENTRY_AUTH_TOKEN` are build-time values; the auth token is a secret (never commit it). For creating the token and wiring it into CI, see `sentry-source-maps` _(unavailable upstream reference omitted)_.
