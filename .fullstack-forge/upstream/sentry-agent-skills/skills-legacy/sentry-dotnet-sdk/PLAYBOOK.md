...
-->
> All Skills _(unavailable upstream reference omitted)_ > SDK Setup _(unavailable upstream reference omitted)_ > .NET SDK
...
Upload needs a `SENTRY_AUTH_TOKEN` set in CI (a secret). For creating the token and the CI wiring, see `sentry-source-maps` _(unavailable upstream reference omitted)_. The `SentryCreateRelease` / `SentrySetCommits` properties also create a release with suspect commits — see `sentry-releases` _(unavailable upstream reference omitted)_.
