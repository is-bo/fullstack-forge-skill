...
-->
> All Skills _(unavailable upstream reference omitted)_ > Feature Setup _(unavailable upstream reference omitted)_ > AI Monitoring
...
Only after explicit user approval for telemetry, prompt/output capture, and the resulting external data transfer, enable telemetry on the approved calls:
```javascript
await generateText({
  model: openai("gpt-4o"),
  prompt: "Hello",
