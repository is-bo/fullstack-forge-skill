...
<!-- fullstack-forge:upstream-reference provider=sentry-agent-skills -->
...
-->
> All Skills _(unavailable upstream reference omitted)_ > Feature Setup _(unavailable upstream reference omitted)_ > OTel Exporter
...
Ask the user to choose and approve an explicit reviewed collector version. Do not query GitHub for the latest release automatically. If the user separately approves network access, verify the chosen immutable release and checksum using the project's normal dependency-review process.
...
Perform these steps only after the user explicitly approves the exact version, network download, package installation, destination, and command execution.
