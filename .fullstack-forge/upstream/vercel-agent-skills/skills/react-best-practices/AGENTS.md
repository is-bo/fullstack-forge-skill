...
This is a specialization of Defer Await Until Needed _(not vendored by Fullstack Forge)_ for `flag && cheapCondition` style checks.
...
For `await getFlag()` combined with a cheap synchronous guard (`flag && someCondition`), see Check Cheap Conditions Before Async Flags _(not vendored by Fullstack Forge)_.
...
For static assets and config, see Hoist Static I/O to Module Level _(not vendored by Fullstack Forge)_.
